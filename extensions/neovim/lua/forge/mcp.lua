...
---@param method string Notification method name (e.g. "forge/warning")
...
function M.start(callback)
  ...
  -- Verify forge exists on PATH before starting the job
  local forge_check = vim.fn.executable("forge")
  if forge_check == 0 then
    state = "error"
    vim.notify(
    ...
    if callback then
      callback("forge binary not found on PATH. Install forge and try again.")
    end
    return
  end
  ...
  job_id = vim.fn.jobstart("forge mcp", {
    ...
    on_exit = function(_, exit_code)
      ...
      if exit_code ~= 0 and next(pending) ~= nil then
        ...
        if stderr_data and (stderr_data:find("not found") or stderr_data:find("No such file")) then
          err_msg = "forge binary not found on PATH. Install forge and try again."
        else
          err_msg = "forge mcp exited unexpectedly (code=" .. exit_code .. "): " .. (stderr_data or "unknown error")
        end
  ...
  if not job_id or job_id <= 0 then
    state = "error"
    vim.notify("Failed to start forge mcp process", vim.log.levels.ERROR)
    if callback then
      callback("Failed to start forge mcp process")
    end
    return
  end
  ...
  M._raw_request("initialize", {
    ...
    capabilities = {},
    client_info = { name = "forge-neovim", version = "0.1.0" },
  }, function(err, _)
    if err then
      state = "error"
      vim.notify("Forge MCP initialize failed: " .. err, vim.log.levels.ERROR)
      if callback then
        callback(err)
      end
      return
...
function M.stop()
  ...
  -- Close stdin (sends EOF to forge mcp)
...
function M._raw_request(method, params, callback)
  ...
  if not ok or ok <= 0 then
    ...
    callback("Failed to write to forge mcp stdin", nil)
  end
end
...
function M._handle_notification(msg)
  ...
  -- Default handling for forge notifications
  if method == "forge/warning" and msg.params then
    local p = msg.params
    if p.finding then
  ...
  elseif method == "forge/error" and msg.params then
    local p = msg.params
    if p.error then
      vim.notify(
        "[Forge Error] " .. (p.operation or "unknown") .. ": " .. p.error,
        vim.log.levels.ERROR
      )
    end
  end
