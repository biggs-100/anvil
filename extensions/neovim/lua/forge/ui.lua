...
--- Show forge environment status in a floating window.
---
--- Sends `prompts/get` with `forge:status` and renders markdown in a float.
---@param mcp table The MCP client module
function M.show_status(mcp)
  if not mcp or not mcp.is_ready() then
    vim.notify("Forge is not connected. Open a project with forge.toml.", vim.log.levels.WARN)
    return
  end

  mcp.get_prompt("forge:status", {}, function(err, result)
    if err then
    ...
    _open_float("Forge Status", text)
  end)
end

--- Run forge diagnostics and populate the quickfix list.
---
--- Sends `prompts/get` with `forge:diagnose` and parses findings into quickfix entries.
---@param mcp table The MCP client module
function M.show_diagnose(mcp)
  ...
  mcp.get_prompt("forge:diagnose", {}, function(err, result)
    if err then
      vim.notify("Forge: " .. err, vim.log.levels.ERROR)
      return
    end
    ...
    vim.notify("Forge diagnose complete — " .. #qf_list .. " finding(s)", vim.log.levels.INFO)
  end)
end

--- Show runtime explanation in a floating window.
---
--- Calls `tools/call` with `forge_explain` and renders the JSON result.
---@param mcp table The MCP client module
...
function M.show_explain(mcp, runtime)
  ...
  mcp.call_tool("forge_explain", { runtime = runtime }, function(err, result)
    if err then
      vim.notify("Forge: " .. err, vim.log.levels.ERROR)
      return
    end
    ...
    _open_float("Forge: Explain " .. runtime, text)
  end)
end

--- Run a command in the forge environment and show output in a terminal buffer.
---
--- Calls `tools/call` with `forge_run`.
---@param mcp table The MCP client module
...
function M.show_run(mcp, input)
  ...
  mcp.call_tool("forge_run", { cmd = cmd, args = args }, function(err, result)
    if err then
      vim.notify("Forge: " .. err, vim.log.levels.ERROR)
      return
    end
    ...
    vim.api.nvim_buf_set_name(buf, "forge://run-output")
...
--- Telescope picker for forge resources.
---
--- Lists available runtimes and config variables from the forge context.
...
function M.telescope_picker(mcp)
  return require("telescope.pickers").new({}, {
    ...
    finder = require("telescope.finders").new_dynamic({
      fn = function()
        -- Fetch forge context and build picker items
        local items = {}
        mcp.call_tool("forge_explain", { runtime = "all" }, function(err, result)
          if err then
            vim.notify("Forge: " .. err, vim.log.levels.ERROR)
            return
          end
    ...
    attach_mappings = function(_, map)
      map("i", "<CR>", function(prompt_bufnr)
        ...
        if selection then
          _open_float("Forge: " .. selection.value, selection.description)
        end
      end)
      return true
    end,
