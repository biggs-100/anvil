...
/**
 * Manages a VS Code WebviewPanel for displaying forge markdown output.
 *
 * Used by the `Forge: Show Status` and `Forge: Explain Runtime` commands to
 * render rich markdown responses from forge prompts and tool calls.
 */
export class ForgePanel {
  ...
  /**
   * Show or reveal the forge panel with the given markdown content.
   *
   * @param title    Panel title (e.g. "Forge: Status", "Forge: Explain")
  ...
  static show(title: string, markdown: string): void {
    if (ForgePanel.currentPanel) {
      ForgePanel.currentPanel._panel.title = title;
      ForgePanel.currentPanel._update(markdown);
      ForgePanel.currentPanel._panel.reveal(vscode.ViewColumn.Beside);
      return;
    }
    ...
    ForgePanel.currentPanel = new ForgePanel(panel);
    ForgePanel.currentPanel._update(markdown);
  }
  ...
  private _renderHtml(markdown: string): string {
    ...
    // This covers the common patterns from forge prompt responses (headings,
