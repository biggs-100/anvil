...
## Purpose
Define the CLI options, output streams, scopes, and exclusion flags for the `anvil context` command interface.

## Requirements

### Requirement: Command Invocation and Options
The CLI MUST support the `anvil context` subcommand with arguments to filter, exclude, and format outputs.
...
- WHEN the user runs `anvil context`
...
- GIVEN a user request `anvil context --scope runtimes,config`
...
- GIVEN a workspace with a `.anvil/cache` directory
- WHEN the user executes `anvil context --exclude-cache`
- THEN the Workspace provider MUST skip scanning files under `.anvil/cache`
