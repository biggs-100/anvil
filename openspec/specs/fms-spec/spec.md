# Forge Manifest Specification v1.0.0 (FMS)

**Version**: 1.0.0 | **Status**: Frozen

## Purpose

Defines file formats and resolution rules for reproducible dev environments: `forge.toml`, `forge.lock`, `forge.env`, `forge.local.toml`, `forge.secrets`, plus resolution and interpolation.
...
key = "forge/prod/DB_PASS"
```

Supported providers: `"keyring"` (OS keyring, scoped by workspace_id), `"file"` (`.forge/secrets.enc`).
...
| 2 | System env | `FORGE_VAR_<KEY>` (prefix stripped) |
| 3 | Local overrides | `forge.local.toml [env]` |
| 2.5 | Plugin providers | ConfigurationProvider values |
| 4 | Secrets | `forge.secrets` resolved |
| 5 | Env file | `forge.env` parsed |
...
- GIVEN `forge.env` has `KEY=env` and CLI passes `--var KEY=cli`
- WHEN resolved
- THEN `KEY` SHALL be `"cli"`

**Scenario**: Local beats profile
- GIVEN `forge.local.toml` sets `KEY=local` and `profile.dev.env` sets `KEY=dev`
...
### Requirement: forge.env SHALL parse correctly
