...
The `ContextEngine` MUST accept `ContextProvider` implementations registered via `PluginRegistry`. Plugin context providers MUST implement the same `ContextProvider` trait as built-in providers and MUST be queried alongside them when building the `AnvilContext`.
...
- THEN the engine MUST query the plugin provider and include its output in the aggregated `AnvilContext`
