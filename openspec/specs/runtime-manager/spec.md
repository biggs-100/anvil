...
| REQ-MGR-004 | The system MUST regenerate `.forge/shims.cache` only after a successful transaction commit/promotion has completed, not after individual runtime installations. | MUST |
| REQ-MGR-005 | The system MUST delegate download and extraction tasks to an isolated staging directory (`.forge/staging/<operation_id>/`) and delay commit until final transaction verification. | MUST |

### Requirement: Runtime Toolchain Management

#### Scenario: Toolchain Cache Hit
- GIVEN Python is cached in `.forge/runtimes` matching the lockfile SHA-256
...
- GIVEN Node is missing from `.forge/runtimes`
