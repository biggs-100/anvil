...
Define the core plugin trait, registry lifecycle (scanning, loading, dependency resolution, initialization), API version gating, and error handling for the Forge plugin system.
...
| REQ-PLUG-002 | PluginRegistry MUST discover plugins from `~/.forge/plugins/` and via `Engine::register_plugin()`. | MUST |
| REQ-PLUG-003 | PluginRegistry MUST reject plugins whose `api_version` does not match `FORGE_PLUGIN_API_VERSION`. | MUST |
...
The registry MUST validate `api_version` against `FORGE_PLUGIN_API_VERSION`. Mismatched plugins MUST be rejected with a descriptive error containing the plugin name, expected version, and actual version.

#### Scenario: API Version Mismatch Rejection
- GIVEN a plugin declaring `api_version = "2.0.0"` and `FORGE_PLUGIN_API_VERSION = "1.0.0"`
...
The registry MUST scan `~/.forge/plugins/` for `.so`/`.dll`/`.dylib` (or recognized plugin artifacts). Invalid entries MUST be skipped with a warning, not a hard failure.

#### Scenario: Plugin Directory Scan
- GIVEN `~/.forge/plugins/` contains one valid plugin artifact and one unrecognized file
