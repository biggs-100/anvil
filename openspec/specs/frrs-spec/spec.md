# Forge Runtime Registry Specification (FRRS)

## Purpose

Define an open, forward-compatible TOML-based format for describing runtime toolchain artifacts, their mirrors, and a registry index. FRRS eliminates hardcoded download URLs by enabling registry-driven resolution.

## Requirements

| ID | Description | Strength |
|---|---|---|
| REQ-FRRS-001 | The system MUST define a `metadata.toml` schema per the structure below. | MUST |
| REQ-FRRS-002 | The system MUST support multiple platforms and architectures per toolchain version. | MUST |
| REQ-FRRS-003 | The system SHOULD include a `mirrors.json` with fallback download URLs. | SHOULD |
| REQ-FRRS-004 | Every FRRS file MUST be forward-compatible — tools MUST ignore unknown fields. | MUST |
| REQ-FRRS-005 | The registry MUST expose an `index.toml` listing all available toolchains with latest versions. | MUST |
| REQ-FRRS-006 | The system MUST resolve a download artifact for a specific platform + arch from metadata. | MUST |
