# Anvil Context Protocol v1.0.0 (ACP)
...
| `method` | string | MUST be `"acp.handshake"` |
...
|----------|-------|--------|
| **Runtime** | `runtime` | Runtimes from anvil.lock: name, version, platform, arch, url, size, sha256 |
| **Configuration** | `configuration` | anvil.toml: workspace_id, runtimes, active_profile, definitions |
...
`name()` + `export(&AnvilContext) -> Result<String, String>`.
...
`name()` + `adapt(&AnvilContext, &dyn ContextExporter) -> Result<String, String>`.

| Adapter | Name | Output |
|---------|------|--------|
| **ClaudeCode** | `"claude"` | XML `<anvil_context>` with runtimes, config, diagnostics, workspace |
...
`is_secret(key)` → true if key contains: secret, key, password, token, auth, credential, pass. Providers MUST replace values with `"[MASKED]"`. No plaintext secrets in AnvilContext.
...
- GIVEN client sends `acp.handshake` with `"1.0.0"`, scopes `["workspace", "secrets"]`, exporters `["json"]`
- WHEN processed
...
- GIVEN `API_KEY=secret123` and `DB_USER=anvil`
- WHEN environment provider collects
- THEN `API_KEY` SHALL be `"[MASKED]"`, `DB_USER` SHALL be `"anvil"`
