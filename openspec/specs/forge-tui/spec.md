# forge-tui Specification

## Purpose

Terminal dashboard for Forge — keyboard-driven Ratatui UI with four read-only views (Dashboard, Runtimes, Diagnostics, History). Shows real-time forge health, runtimes, diagnostic findings, and operation history without mutating engine state.
...
The Runtimes view MUST show all runtimes from forge.toml with name, version, state, lock state, cache path, and shim status. SHOULD colorize by state. MUST scroll with `j`/`k` and arrows.

#### Scenario: Three runtimes listed

- GIVEN three runtimes configured in forge.toml
- WHEN user opens Runtimes view
- THEN all three display with complete metadata

#### Scenario: No runtimes configured

- GIVEN forge.toml defines zero runtimes
- WHEN user opens Runtimes view
...
- GIVEN forge-tui is running
- WHEN user presses `q`
- THEN app exits and terminal is restored to original state

#### Scenario: Terminal resize

- GIVEN forge-tui is running at 120×40
- WHEN terminal resizes to 80×24
- THEN UI re-renders to fit new dimensions without errors

### Requirement: Error Handling

MUST handle engine errors inline — display error message, never panic. MUST show error state if no forge.toml found. MUST NOT panic on empty data in any view.

#### Scenario: Missing forge.toml

- GIVEN no forge.toml exists in the current directory
- WHEN user launches forge-tui
- THEN "No forge.toml found" error message is shown in the dashboard
