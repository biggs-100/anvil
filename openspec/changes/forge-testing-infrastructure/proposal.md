# Proposal: Anvil Testing Infrastructure
...
- Audit anvil-core sources for untested modules; add baseline coverage
...
### New Capabilities
- `testing-infrastructure`: CI/CD pipeline definition and integration test framework for anvil-cli
...
**Phase 3**: Audit `anvil-core/src/` — files without `#[cfg(test)]` (launcher.rs, lock.rs, manifest.rs, operations/mod.rs). Add smoke tests or TODO markers. Report coverage delta.

## Affected Areas

| Area | Impact | Description |
|------|--------|-------------|
| `crates/anvil-cli/tests/jsonrpc_test.rs` | Modified | Remove `#[ignore]`, ensure `CARGO_BIN_EXE_` always resolves |
| `crates/anvil-cli/tests/mcp_test.rs` | Modified | Same as above |
| `.github/workflows/ci.yml` | New | Matrix build+test on 3 OSes |
| `anvil-core/src/*.rs` | Modified | Add test modules for untested files |
...
- [ ] anvil-core source files without tests identified and documented
