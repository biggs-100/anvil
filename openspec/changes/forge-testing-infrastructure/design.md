# Design: Forge Testing Infrastructure

## Technical Approach

Three-phase rollout: (1) un-ignore integration tests by fixing the `CARGO_BIN_EXE_` env var resolution and adding a platform guard for the kill-based lifecycle test, (2) add a GitHub Actions matrix CI workflow with caching and clippy enforcement, (3) document forge-core coverage gaps with TODO markers — no automatic test generation.

The key insight: Cargo already sets `CARGO_BIN_EXE_FORGE_CLI` (uppercase, underscore) for integration tests in a package that produces a binary. The tests' current fallback chain checks for wrong var names and never matches, forcing `#[ignore]`. Fix the var name and the tests run automatically.

## Architecture Decisions

| Decision | Choice | Alternatives | Rationale |
|----------|--------|-------------|-----------|
| Binary resolution env var | `CARGO_BIN_EXE_FORGE_CLI` | Current lowercase-hyphen `CARGO_BIN_EXE_forge-cli` | Cargo uppercases and underscores binary names — current code never matches, so `#[ignore]` was the only way to avoid failures |
...
│                  forge-cli.exe    tests  tests (11)          │
│                                     │                        │
└─────────────────────────────────────┼────────────────────────┘
                                      │
                    CARGO_BIN_EXE_FORGE_CLI ──→ spawn forge-cli
                                                  │
                                              stdin/stdout
                                                  │
                                              JSON-RPC 2.0
...
Integration test flow: `cargo test` → Cargo sets `CARGO_BIN_EXE_FORGE_CLI` env var → `forge_exe()` resolves binary path → `Command::new()` spawns `forge jsonrpc` or `forge mcp` → test writes JSON-RPC request to stdin → reads response from stdout → asserts expected fields.
...
| `crates/forge-cli/tests/jsonrpc_test.rs` | Modify | Fix `CARGO_BIN_EXE_forge-cli` → `CARGO_BIN_EXE_FORGE_CLI`; remove `#[ignore]` from all 5 tests; add `#[cfg(not(target_os = "windows"))]` to `test_subprocess_lifecycle_error` |
| `crates/forge-cli/tests/mcp_test.rs` | Modify | Fix `CARGO_BIN_EXE_forge-cli` → `CARGO_BIN_EXE_FORGE_CLI`; remove `#[ignore]` from all 6 tests |
| `crates/forge-core/src/launcher.rs` | Modify | Add `// TODO: tests — requires mocking OS process APIs` at module root |
| `crates/forge-core/src/lock.rs` | Modify | Add `// TODO: tests — requires filesystem isolation fixture` at module root |
| `crates/forge-core/src/manifest.rs` | Modify | Add `// TODO: tests — requires TOML parsing fixtures` at module root |
| `crates/forge-core/src/state.rs` | Modify | Add `// TODO: tests — depends on Engine lifecycle` at module root |
| `crates/forge-core/src/lib.rs` | Modify | Add `// TODO: tests — re-export facade; test coverage in submodules` |
| `crates/forge-core/src/operations/mod.rs` | Modify | Add `// TODO: tests — operations module` |
| `crates/forge-core/src/api/mod.rs` | Modify | Add `// TODO: tests — API surface is tested via v1.rs integration` |
| `crates/forge-core/src/plugin/mod.rs` | Modify | Add `// TODO: tests — registry.rs has unit tests; plugin lifecycle needs e2e` |
...
3. Add TODO markers to forge-core modules
...
- [ ] Verify `CARGO_BIN_EXE_FORGE_CLI` casing locally — Cargo docs say uppercase+underscore, but empirical check on the actual toolchain is needed
