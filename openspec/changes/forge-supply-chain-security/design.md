# Design: Forge Supply Chain Security

## Technical Approach

Three independent capabilities layered on existing forge pipeline points — GPG verification at metadata fetch, pin-by-hash at manifest parse + install, and audit at display. No changes to core resolution/lock/sync orchestration.
...
| Key source | Embedded `const &str` + `FORGE_TRUSTED_KEYS` env var | Static key avoids runtime fetch; env var allows user override |
...
| Audit output | Table (default) + `--json` | Matches `forge history` pattern |
...
### Pin-by-Hash
```
forge.toml → Deserialize runtimes
...
|------|--------|-------------|
| `crates/forge-core/src/gpg.rs` | Create | Embedded key constant, `verify_gpg_signature()` fn |
| `crates/forge-core/src/registry.rs` | Modify | Fetch `.asc`, call verifier in `fetch_metadata()` |
| `crates/forge-core/src/manifest.rs` | Modify | `RuntimeEntry` enum, `HashMap<String, RuntimeEntry>` |
| `crates/forge-core/src/installer.rs` | Modify | Manifest-pin hash check before extraction |
| `crates/forge-core/src/types.rs` | Modify | Add optional audit fields to `Event` |
| `crates/forge-cli/src/audit.rs` | Create | Audit command handler (read journal, format table/JSON) |
| `crates/forge-cli/src/main.rs` | Modify | Add `Audit` variant, wire in `run_cli()`, add to `BUILTIN_COMMANDS` |
...
| Integration | `forge audit` empty journal | No `.forge/journal.jsonl`, assert "no operations" |
| Integration | GPG fallback to cache | Mock HTTP returns bad sig, assert cached data served |

## Migration / Rollout

No data migration. All three features are opt-in/additive: GPG signs server-side first (client tolerates missing sig in lenient mode), pin-by-hash is an optional forge.toml field, audit is a new command reading existing journal. Rollback is revert of the relevant files.

## Open Questions

- [ ] `gpg` CLI path detection — probe once at startup or on each verify call?
- [ ] Strict vs lenient mode — env var `FORGE_GPG_STRICT=1` or config setting?
