...
Decouple toolchain providers and archive extraction mechanisms through structured traits. An offline-first local hybrid registry metadata database (`.forge/metadata_cache.toml`) will handle offline semver matching. Windows ARM64 missing architectures fall back to emulated x86_64 versions, logging details to `forge.lock`. A Tokio `JoinSet` handles asynchronous concurrent download, verification, and extraction pipelines.

## Architecture Decisions

| Decision / Option | Tradeoffs | Selected Choice & Rationale |
| :--- | :--- | :--- |
| **Offline Version Resolution**: Static registry vs. Hybrid Registry | Static has zero runtime overhead but requires CLI updates. Hybrid is complex but matches offline using cached entries. | **Hybrid Registry**: Uses local cache `.forge/metadata_cache.toml` with remote fallback to allow updates. |
...
| **Windows ARM64 Fallback**: Hard fail vs. Emulated execution | Failing blocks developers on ARM64. Emulation via x86_64 keeps them operational with transparent audit logs. | **Emulated x86_64 Fallback**: Fallback to x86_64 binaries and log emulation details to `forge.lock`. |

## Data Flow

```text
[forge.toml] ──> [Runtime Manager] ──> [Provider] ──> [Hybrid Registry] (local/remote)
                       │
                       ...
                       ▼
                 [forge.lock] (Updated with Runtime Info & Emulation Logs)
```
...
|------|--------|-------------|
| `crates/forge-core/Cargo.toml` | Modify | Add `xz2` dependency for TarXz extraction. |
| `crates/forge-core/src/lib.rs` | Modify | Add `Extractor` and `Provider` traits, implementation structs, and `HybridRegistry`. |
| `crates/forge-core/src/lock.rs` | Create | Refactor `Lockfile` and serialization logic, adding `EmulationLog`. |
...
### Emulation Log Structure in `forge.lock`
```rust
...
2. Load entries from `.forge/metadata_cache.toml`.
...
| Integration | Lockfile Serialization | Verify Windows ARM64 fallback triggers correct `emulation` fields in `forge.lock`. |

## Migration / Rollout

No migration required. If the local metadata cache is corrupted, delete `.forge/metadata_cache.toml` to trigger rebuild.
