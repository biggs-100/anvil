# Design: Anvil Context Platform (ACP)

## Technical Approach

ACP implements a trait-based query framework inside `anvil-core` to aggregate project state, sanitizing secrets and filtering filesystem layouts, before exporting to CLI/JSON-RPC (MCP) or adapting to custom agent prompt wrappers.
...
|------|--------|-------------|
| `crates/anvil-core/src/context/mod.rs` | Create | ACP Core types, traits, engine, and 6 providers. |
| `crates/anvil-core/src/lib.rs` | Modify | Re-export `context` module. |
| `crates/anvil-cli/src/main.rs` | Modify | Hook up `anvil context` subcommand and options parsing. |

## Interfaces / Contracts

```rust
// crates/anvil-core/src/context/mod.rs
...
1. **Runtime**: Summarizes active runtimes/shims from `anvil.lock`.
2. **Configuration**: Parses `anvil.toml` active profile configuration.
...
|-------|-------------|----------|
| **Unit** | ACP Handshake | Validate JSON-RPC serialization & capability intersection. |
...
| **Integration** | CLI Outputs | Test `anvil context --format json` and `--format markdown` write valid syntax. |
