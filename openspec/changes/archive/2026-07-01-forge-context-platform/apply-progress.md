...
- [x] **Task 1.1**: Define traits `ContextProvider`, `ContextExporter`, `AgentAdapter` in `crates/anvil-core/src/context/mod.rs`.
- [x] **Task 1.2**: Implement `ContextEngine` registry, capability negotiation handshake structs (JSON-RPC), and `ForgeContext` schema.
- [x] **Task 1.3**: Update `crates/anvil-core/src/lib.rs` to re-export the `context` module and core structs.
...
- [x] **Task 3.3**: Add the `context` command to `Commands` enum in `crates/anvil-cli/src/main.rs` and route execution to `ContextEngine`.
- [x] **Task 3.4**: Create CLI integration tests in `crates/anvil-cli/tests/context_cli_tests.rs` verifying dry-runs and output formats.

## Created & Modified Files

| File | Action | Description |
|------|--------|-------------|
| `crates/anvil-core/src/context/mod.rs` | Created | ACP Core types, traits, engine, 6 providers, exporters, adapters, and unit tests. |
| `crates/anvil-core/src/lib.rs` | Modified | Declared and re-exported `context` module. |
| `crates/anvil-cli/src/main.rs` | Modified | Declared and integrated top-level `context` command. |
| `crates/anvil-cli/tests/context_cli_tests.rs` | Created | Integration tests verifying CLI help, JSON, and Markdown outputs. |
