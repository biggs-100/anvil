# Archive Report: Anvil Bootstrap
...
- **Phase 1: Workspace & Configuration (PR 1)**
  - Create workspace root `Cargo.toml` and crates (`anvil-cli`, `anvil-core`, `anvil-drivers`).
  - Define `anvil.toml` manifest structs with `serde` / `toml`.
  - Implement `anvil.env` parser with masked credentials helper.
- **Phase 2: Runtime Downloader & Cache (PR 2)**
  - Set up global cache path (`~/.anvil/runtimes/`) and lockfile generator.
...
- **Phase 4: CLI Interface & AI Diagnostics (PR 4)**
  ...
  - Implement `anvil ai context` command displaying JSON map with masked secrets.
  - Implement `anvil ai doctor` diagnostic checks.
