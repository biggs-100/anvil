...
|------|------|-----------|-------|
| 1 | JSON-RPC server in anvil-cli: subcommand, read-dispatch-write loop, method catalog, error codes | PR 1 | Base branch: main. Foundation all SDKs depend on |
| 2 | Rust SDK crate: Anvil struct, typed methods, async feature | PR 2 | Depends on anvil-core only — independent from PR 1 |
| 3 | Go SDK: go.mod, client.go, types.go, subprocess + JSON-RPC | PR 3 | Depends on PR 1 (needs jsonrpc mode in anvil-cli) |
...
- [x] 1.1 Add `JsonRpc` variant to `Commands` enum in `crates/anvil-cli/src/main.rs`
- [x] 1.2 Create `crates/anvil-cli/src/jsonrpc.rs` — stdin read-dispatch-write loop with tokio
...
## Phase 2: SDK Rust — anvil-sdk Crate

- [x] 2.1 Create `crates/anvil-sdk/Cargo.toml`, add to `[workspace].members` in root `Cargo.toml`
- [x] 2.2 Create `crates/anvil-sdk/src/types.rs` — `ForgeError` (Display + Error), type aliases
- [x] 2.3 Create `crates/anvil-sdk/src/lib.rs` — `Forge` struct wrapping `Engine` directly
...
- [x] 4.1 Create `sdks/python/pyproject.toml` — package `anvil-sdk`, stdlib only
...
- [x] 6.1 Integration test: spawn `anvil jsonrpc`, send JSON-RPC requests, verify responses
- [x] 6.2 anvil-sdk crate tests — `cargo test -p anvil-sdk` covering all typed methods
...
- [x] 6.5 TypeScript SDK integration test — Node.js script spawning `anvil jsonrpc`, verifying JSON-RPC responses (uses `child_process` directly, no test runner dependency)
- [~] 6.6 Cross-SDK parity test — deferred (requires CI matrix for all 4 SDK binaries)
- [x] 6.7 Subprocess lifecycle error test — forge-kill-mid-request test in `crates/anvil-cli/tests/jsonrpc_test.rs`
