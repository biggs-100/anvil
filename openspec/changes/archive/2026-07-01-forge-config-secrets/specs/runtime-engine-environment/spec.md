...
The runtime engine MUST materialize process environments by executing the 5-layered configuration resolver, combining manifest configurations, environment files, secrets, local overrides, and CLI overrides instead of parsing `anvil.env` in isolation.
