...
## Intent
Implement Phase 7 (Diagnostic Engine) to build a sovereign diagnostics platform for Anvil sandboxes. Replaces sequential CLI validation logic with a parallelized DAG-based engine executing 11 checks concurrently, computing a health score, and generating repair plans via a decoupled RepairPlanner.

## Scope

### In Scope
- Define `HealthCheck` trait and `DiagnosticEngine` in `crates/anvil-core/src/diagnostics/mod.rs`.
...
- Add CLI commands `anvil doctor [--deep] [--json]` and `anvil ai doctor` (masking all secrets).
...
|------|--------|-------------|
| `crates/anvil-core/src/diagnostics/mod.rs` | New | Module for traits, engine, and 11 checks. |
| `crates/anvil-core/src/lib.rs` | Modified | Export diagnostics module. |
| `crates/anvil-core/src/operations/mod.rs` | Modified | Add `RepairPlanner` mapping to `RepairPlan`. |
| `crates/anvil-cli/src/main.rs` | Modified | Route `anvil doctor [--deep] [--json]` and `anvil ai doctor`. |
...
## Rollback Plan
- Revert code changes to `crates/anvil-cli` and `crates/anvil-core` using git. No persistent database state is modified.
...
- [ ] `anvil doctor --json` prints valid JSON with zero plaintext secret leakage.
