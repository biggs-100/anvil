...
To capture and persist domain events from the `EventBus` into the local Operation Journal file (`.forge/journal.jsonl`), we will establish a structured journal logging component.

#### Implementation Design
- **Path**: `.forge/journal.jsonl` under the workspace root.
...
- **Mechanism**:
  ...
  - To prevent filesystem blocking on the main asynchronous runtime, a background worker task will use a buffered channel to write events to `.forge/journal.jsonl`.
  - We use standard append mode (`std::fs::OpenOptions::new().create(true).append(true).open(...)`) to ensure atomic writes.

```rust
// crates/forge-core/src/event_bus.rs (Proposed additions)
use std::path::Path;
...
### 2. CLI Command: `forge history`
The `forge history` command allows users to inspect a log of past sandbox operations in the active workspace.
...
- **Aggregating Events into Operations**:
  - Scan `.forge/journal.jsonl` line-by-line.
...
### 3. CLI Command: `forge explain <runtime>`
The `forge explain <runtime>` command provides detailed troubleshooting information explaining how a specific runtime's version was resolved, where it's cached, and its metadata.

#### Output Details
- **Config Source**:
  - Path of `forge.toml` currently active.
...
- **Version Resolution Rules**:
  ...
  - Version selection candidates list found in registry (e.g. `.forge/metadata_cache.toml` or internal fallback).
...
- **Cache & Downloads**:
  - Cache storage location (e.g., `~/.forge/runtimes/node/20.10.0/`).
...
### 4. CLI Command: `forge trace <operation_id>`
The `forge trace <operation_id>` command reads the operation journal and reconstructs a chronological, step-by-step trace of that operation.

#### CLI Output Design
- Read `.forge/journal.jsonl` and filter events by the given `operation_id`.
...
#### Recommendation
**Option B: Journal File Tailing** is highly recommended. It keeps the architecture clean and simple. The CLI command `forge events` can just watch `.forge/journal.jsonl` using a tail watcher (using standard file polling or the `notify` crate). If the user specifies `--live`, it starts at the end of the file and prints new line appends in real-time.

---

### 6. Stable Public API Interface Facade
To freeze the public interfaces for integrations (such as IDE extensions or automated scripts), we will design a stable API facade module under `crates/forge-core/src/api/v1.rs`.

#### Module Architecture
- `crates/forge-core/src/lib.rs` will export `pub mod api;`
- `crates/forge-core/src/api/mod.rs` will export `pub mod v1;`
- `crates/forge-core/src/api/v1.rs` contains the high-level struct `Engine` representing the sovereign sandbox engine runtime.
...
impl Engine {
    ...
    pub fn read_history(&self, limit: Option<usize>) -> Result<Vec<OperationSummary>, String> {
        // reads and groups .forge/journal.jsonl lines
        unimplemented!()
    }
...
- **Decision**: Implement a command pattern trait `Operation` paired with an execution `Context` and a `Plan`. This separates resolution/planning from actual system modification, enabling dry-runs (`forge plan`).
- **Status**: Accepted.

#### ADR-0002: Lifecycle Management
- **Context**: The engine must keep track of local sandbox state.
- **Decision**: Define a formal `LifecycleState` state-machine. Persist the current computed state in `.forge/state.json`. Verify transitions on every operation.
- **Status**: Accepted.
...
- **Decision**: Implement all installers using a staging-and-promote transactional pattern. Perform downloads and extracts into `.forge/staging/`. Promote via directory rename. Perform backup renames for existing items and rollback to backup in case of failures.
- **Status**: Accepted.
...
- **Decision**: Use a broadcast event channel in memory during execution. Persist events concurrently to `.forge/journal.jsonl` as JSON Lines. Allow CLI live watching by tailing the journal file.
- **Status**: Accepted.

#### ADR-0006: Shim Architecture & Path Hijacking
- **Context**: Sandboxed tools must execute seamlessly without modifying user files.
- **Decision**: Generate lightweight wrappers (shims) inside `.forge/bin/` pointing to the shim binary. The shim uses a generated signature cache `.forge/shims.cache` to locate the correct target binary version for the workspace, modifying the execution path (`PATH`) on the fly.
- **Status**: Accepted.

---

### Affected Areas
- `crates/forge-core/src/event_bus.rs` — Incorporate journal persist hooks.
- `crates/forge-core/src/api/v1.rs` — Create the new stable API facade interface.
- `crates/forge-core/src/lib.rs` — Re-export the API facade modules.
- `crates/forge-cli/src/main.rs` — Add subcommands for `history`, `explain`, `trace`, and `events`.
- `docs/adr/` — Create ADRs 0001 through 0006.

### Recommendation
1. Implement the journal capture system using an async logger task listening to the `EventBus` and writing directly to `.forge/journal.jsonl`.
2. Standardize `forge events` on tailing the `.forge/journal.jsonl` file as it keeps implementation complexity low and provides a single unified codebase path.
3. Expose all command logic via the stable public facade `Engine` inside `crates/forge-core/src/api/v1.rs` to allow robust CLI integration and third-party programmatic consumption.
...
- **Journal Bloat**: Frequent command executions (especially in CI/CD) could grow the `.forge/journal.jsonl` size. Mitigate by implementing a size cap or simple retention policy (e.g. discard lines older than 30 days during clean/gc operations).
- **Process Concurrency**: Multiple forge instances writing to the same journal file simultaneously could interleaving characters. Mitigate by locking the file or using platform-level atomic appends for small payloads.
