# Implementation Progress: Anvil Observability & Telemetry
...
|------|--------|-------------|
| `crates/anvil-core/src/api/v1.rs` | Created | Stable API facade exposing `Engine` with `sync`, `repair`, `clean`, `explain`, `history`, `trace`, and `events` APIs. |
| `crates/anvil-core/src/api/mod.rs` | Created | Export sub-module `v1`. |
| `crates/anvil-core/src/lib.rs` | Modified | Re-exported stable `api::v1` namespace and types. |
| `crates/anvil-core/src/event_bus.rs` | Modified | Spawns background tokio task for NDJSON `.anvil/journal.jsonl` writes; added safe `new_with_journal` and unit tests. |
| `crates/anvil-cli/src/main.rs` | Modified | Added subcommands `history`, `explain`, `trace`, `events`. Remapped other execution handlers to `Engine`. Added table and ASCII tree formatting. |
...
| `crates/anvil-core/tests/integration.rs` | Modified | Added integration tests verifying `explain` resolution, `trace` ASCII formatting, and `events` live tailing. |
