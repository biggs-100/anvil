...
### Requirement: Policy Section in forge.toml

The `forge.toml` manifest MUST support an optional `[policy]` section for declarative pre-flight policy rules. The section is parsed by `PolicyEngine` and is independent of the existing `[config]` and `[profile]` sections.
...
- GIVEN `forge.toml` contains `[policy]` with `allow_network = false`
...
- GIVEN `forge.toml` does not contain a `[policy]` section
- WHEN the manifest is parsed
