...
Add `forge benchmark` as a clap subcommand in forge-cli. All 5 benchmarks call existing Engine/DiagnosticEngine/ContextEngine APIs wrapped in `std::time::Instant`. Benchmark logic lives in a new `crates/forge-cli/src/benchmark.rs` module to keep `main.rs` clean. Results printed as a formatted table; `--json` outputs `serde_json::Value`.
...
| Module in forge-cli vs forge-core | forge-core already exposes all needed APIs; no new core logic needed | forge-cli — keeps core free of CLI concerns |
...
|------|--------|-------------|
| `crates/forge-cli/src/benchmark.rs` | Create | Benchmark runner — 5 benchmark functions + table/JSON formatting |
| `crates/forge-cli/src/main.rs` | Modify | Add `mod benchmark;`, `Commands::Benchmark` variant, and dispatch in `run_cli` |
...
| Integration | All 5 benchmarks run without panic | `cargo test` in a temp forge project dir |
| Integration | Error per-benchmark (bad project dir) | Run in a non-forge directory, confirm remaining benchmarks continue |
