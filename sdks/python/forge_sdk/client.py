"""Forge Python SDK — subprocess-based JSON-RPC client."""
...
class ForgeError(Exception):
    """Error raised by forge-sdk operations.
...
class Forge:
    ...
    def __init__(self, forge_path: str = "forge"):
        """Spawn the forge jsonrpc subprocess.

        Args:
            forge_path: Path to the forge binary (default: "forge").
        """
    ...
    def close(self) -> None:
        """Terminate the forge subprocess."""
    ...
    def _call(self, method: str, params: Optional[dict[str, Any]] = None) -> Any:
        ...
        if self._process.poll() is not None:
            raise ForgeError("forge subprocess is not running")
        ...
        if not response_line:
            raise ForgeError("forge subprocess closed connection unexpectedly")

        response = json.loads(response_line)
        if "error" in response and response["error"] is not None:
            err = response["error"]
            raise ForgeError(err.get("message", "Unknown error"), code=err.get("code"))
