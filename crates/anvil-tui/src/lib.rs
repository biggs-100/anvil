//! anvil-tui — Terminal dashboard for Anvil environments.
//!
...
use anvil_core::{
    api::v1::OperationSummary, find_anvil_toml, load_config, load_lockfile, DiagnosticContext,
...
impl App {
    ...
    async fn fetch_dashboard_data(&mut self) {
        ...
        let toml_path = find_anvil_toml(&self.diag_ctx.workspace_root);
        let config_error = if toml_path.is_none() {
            Some("No anvil.toml found".to_string())
        } else {
            None
        };
    ...
    async fn fetch_runtimes_data(&mut self) {
        let toml_path = match find_anvil_toml(&self.diag_ctx.workspace_root) {
            Some(p) => p,
            None => {
                self.runtimes_data = RuntimesData {
                    runtimes: Vec::new(),
                    error: Some("No anvil.toml found".to_string()),
                };
                return;
            }
        };
        ...
        let config = match load_config(&toml_path) {
            ...
            Err(e) => {
                self.runtimes_data = RuntimesData {
                    runtimes: Vec::new(),
                    error: Some(format!("Failed to parse anvil.toml: {e}")),
                };
                return;
            }
        };

        let lock_path = self.diag_ctx.workspace_root.join("anvil.lock");
...
mod tests {
    ...
    fn test_tab_switching_via_handle_event() {
        ...
        let tmp = std::env::temp_dir().join("anvil_tui_test_tab_switch");
    ...
    #[test]
    fn test_quit_key() {
        let tmp = std::env::temp_dir().join("anvil_tui_test_quit");
        let _ = std::fs::create_dir_all(&tmp);
        let engine = Engine::new(tmp.clone()).unwrap();
        let cache_dir = tmp.join("cache");
        let _ = std::fs::create_dir_all(&cache_dir);
    ...
    fn test_scroll_state() {
        let tmp = std::env::temp_dir().join("anvil_tui_test_scroll");
        let _ = std::fs::create_dir_all(&tmp);
        let engine = Engine::new(tmp.clone()).unwrap();
        let cache_dir = tmp.join("cache");
        let _ = std::fs::create_dir_all(&cache_dir);
    ...
    fn test_resize_event() {
        let tmp = std::env::temp_dir().join("anvil_tui_test_resize");
        let _ = std::fs::create_dir_all(&tmp);
        let engine = Engine::new(tmp.clone()).unwrap();
        let cache_dir = tmp.join("cache");
        let _ = std::fs::create_dir_all(&cache_dir);
