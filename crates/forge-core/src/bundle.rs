...
impl std::fmt::Display for BundleError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BundleError::MissingForgeToml => {
                write!(f, "forge.toml not found")
            }
...
fn build_metadata(forge_toml: &str, created_at: &str) -> BundleMetadata {
    let runtime_count = match forge_toml.parse::<toml::Value>().ok() {
        Some(v) => v
            .get("runtimes")
...
#[cfg(test)]
const EXCLUDED_NAMES: &[&str] = &[".forge", "forge.secrets", "forge.env"];
...
fn write_archive(
    ...
    let mut encoder = GzEncoder::new(output_file, Compression::default());
    {
        ...
        // Entries in sorted order: forge.toml, forge.lock, metadata.json, bundle.sha256
        add_file_to_tar(&mut archive, "forge.toml", forge_toml_bytes)?;
        add_file_to_tar(&mut archive, "forge.lock", forge_lock_bytes)?;
...
/// Create a deterministic `.forge` archive from a workspace.
///
/// 1. Reads `forge.toml` and `forge.lock` from `workspace_dir`.
...
pub fn create_bundle(workspace_dir: &Path, output_path: &Path) -> Result<(), BundleError> {
    ...
    let toml_path = workspace_dir.join("forge.toml");
    if !toml_path.exists() {
        return Err(BundleError::MissingForgeToml);
    }

    // --- Read workspace files ---
    let forge_toml = std::fs::read_to_string(&toml_path)?;
    let forge_toml_bytes = forge_toml.as_bytes();

    let lock_path = workspace_dir.join("forge.lock");
    let forge_lock = if lock_path.exists() {
        std::fs::read_to_string(&lock_path)?
    } else {
        String::new()
    };
    let forge_lock_bytes = forge_lock.as_bytes();

    // --- Build metadata ---
    let created_at = format_iso_timestamp();
    let metadata = build_metadata(&forge_toml, &created_at);
    let metadata_json = serde_json::to_string_pretty(&metadata)?;
    let metadata_bytes = metadata_json.as_bytes();

    // --- Compute SHA-256 per entry ---
    let toml_hash = sha256_hex(forge_toml_bytes);
    let lock_hash = sha256_hex(forge_lock_bytes);
    let meta_hash = sha256_hex(metadata_bytes);

    // --- Build manifest ---
    let mut manifest = HashMap::new();
    manifest.insert("forge.toml".to_string(), toml_hash);
    manifest.insert("forge.lock".to_string(), lock_hash);
    ...
    // --- Write archive ---
    write_archive(output_path, forge_toml_bytes, forge_lock_bytes, metadata_bytes, sha256_bytes)
}
...
pub fn restore_bundle(bundle_path: &Path, workspace_dir: &Path) -> Result<(), BundleError> {
    ...
    let temp_dir = workspace_dir.join(".forge").join(".bundle_extract");
    if temp_dir.exists() {
    ...
    // --- Check that forge.toml exists in the extracted content ---
    let extracted_toml = temp_dir.join("forge.toml");
    if !extracted_toml.exists() {
        let _ = std::fs::remove_dir_all(&temp_dir);
        return Err(BundleError::MissingEntry("forge.toml".to_string()));
    }

    // --- Atomic rename forge.toml and forge.lock ---
    let dest_toml = workspace_dir.join("forge.toml");
    let dest_lock = workspace_dir.join("forge.lock");
    ...
    let extracted_lock = temp_dir.join("forge.lock");
...
mod tests {
    ...
    fn write_forge_toml(ws: &Path, content: &str) {
        std::fs::write(ws.join("forge.toml"), content).unwrap();
    }

    fn write_forge_lock(ws: &Path, content: &str) {
        std::fs::write(ws.join("forge.lock"), content).unwrap();
    }
    ...
    fn test_deterministic_bundle() {
        ...
        let out1 = ws.join("out1.forge");
        let out2 = ws.join("out2.forge");
    ...
    fn test_verify_checksums_ok() {
        let (_ctx, ws) = TestContext::new();
        write_forge_toml(&ws, "[runtimes]\ntest = \"*\"\n");
        write_forge_lock(&ws, "[test]\nversion = \"1.0\"\n");

        let bundle_path = ws.join("test.forge");
    ...
    fn test_verify_checksums_mismatch() {
        ...
        // Tamper with forge.toml
        std::fs::write(extract.join("forge.toml"), "tampered content").unwrap();
    ...
    fn test_verify_checksums_missing_entry() {
        ...
        // Delete forge.lock
        let _ = std::fs::remove_file(extract.join("forge.lock"));
    ...
    fn test_is_excluded_paths() {
        assert!(is_excluded(Path::new(".forge/metadata_cache/file.toml")));
        assert!(is_excluded(Path::new("forge.secrets")));
        assert!(is_excluded(Path::new("forge.env")));
        assert!(is_excluded(Path::new(".forge/something")));
        assert!(!is_excluded(Path::new("forge.toml")));
        assert!(!is_excluded(Path::new("forge.lock")));
    ...
    fn test_secrets_not_in_archive_entries() {
        ...
        std::fs::write(ws.join("forge.secrets"), "secret_key=abc123").unwrap();
        std::fs::write(ws.join("forge.env"), "MY_VAR=value").unwrap();
        std::fs::write(ws.join(".forge/cache.toml"), "data").unwrap();
        ...
        assert!(extract.join("forge.toml").exists());
        assert!(extract.join("forge.lock").exists());
        assert!(extract.join("metadata.json").exists());
        assert!(extract.join("bundle.sha256").exists());

        // These should NOT be present
        assert!(!extract.join(".forge").exists());
        assert!(!extract.join("forge.secrets").exists());
        assert!(!extract.join("forge.env").exists());
    }
    ...
    fn test_restore_bundle_roundtrip() {
        ...
        write_forge_toml(&ws, toml_content);
        write_forge_lock(&ws, lock_content);

        let bundle_path = ws.join("project.forge");
        create_bundle(&ws, &bundle_path).unwrap();
        ...
        let restored_toml = std::fs::read_to_string(restore_dir.join("forge.toml")).unwrap();
        let restored_lock = std::fs::read_to_string(restore_dir.join("forge.lock")).unwrap();
    ...
    fn test_restore_bundle_no_lock() {
        let (_ctx, ws) = TestContext::new();
        write_forge_toml(&ws, "[runtimes]\nnone = \"*\"\n");
        // No forge.lock written

        let bundle_path = ws.join("noforge.forge");
        create_bundle(&ws, &bundle_path).unwrap();
        ...
        assert!(restore_dir.join("forge.toml").exists());
    }
    ...
    fn test_restore_bundle_tampered_rejected() {
        ...
        // extract, modify forge.toml, re-pack, then restore should fail checksum.
        ...
        // Modify forge.toml content (but NOT bundle.sha256)
        std::fs::write(extract.join("forge.toml"), "tampered content").unwrap();

        // Re-pack the tampered content into a new archive
        let tampered_path = ws.join("tampered.forge");
        {
            ...
            let mut encoder = GzEncoder::new(out, Compression::default());
            {
                ...
                for entry_name in &["forge.toml", "forge.lock", "metadata.json", "bundle.sha256"] {
        ...
        // Should fail — checksum mismatch for forge.toml
    ...
    #[test]
    fn test_bundle_without_forge_toml_errors() {
        let (_ctx, ws) = TestContext::new();
        // No forge.toml

        let bundle_path = ws.join("empty.forge");
        let result = create_bundle(&ws, &bundle_path);
        assert!(matches!(result, Err(BundleError::MissingForgeToml)));
    }
