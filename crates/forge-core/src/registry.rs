...
/// An artifact entry within an FRRS metadata.toml — describes a
...
/// Full FRRS metadata.toml for a single toolchain version.
...
/// Fetches FRRS metadata from a remote registry over HTTP and
...
impl RemoteRegistry {
    ...
    /// Cache is stored under `cache_dir` using the FRRS directory layout
    ...
    pub async fn fetch_metadata(&self, name: &str, version: &str) -> Result<FrrsMetadata, String> {
        ...
        if let Err(e) = self.save_to_cache(&metadata) {
            eprintln!("[forge] Warning: Failed to cache {}/{}: {}", name, version, e);
        }
    ...
    /// FRRS cache directory.  Returns `None` when the file is
    /// missing or corrupt.
    ///
    /// TTL semantics:
    ...
    pub fn load_cached(&self, name: &str, version: &str) -> Option<FrrsMetadata> {
        ...
        if let Ok(meta) = fs::metadata(&path) {
            if let Ok(modified) = meta.modified() {
                if let Ok(elapsed) = SystemTime::now().duration_since(modified) {
                    if elapsed >= self.ttl {
                        eprintln!(
                            "[forge] Warning: Cached {}/{} is stale ({}h old), serving anyway",
                            name,
                            version,
    ...
    fn save_to_cache(&self, metadata: &FrrsMetadata) -> Result<(), String> {
        let path = self
            .cache_dir
            .join(&metadata.name)
...
impl HybridRegistry {
    ...
    /// Attach a remote registry for FRRS HTTP fetches.
    pub fn with_remote(mut self, remote: RemoteRegistry) -> Self {
        self.remote = Some(remote);
        self
    }

    /// Set a local FRRS cache directory for offline metadata lookup.
    ...
    /// Ensure the FRRS cache is populated for `name` by fetching the
    ...
    pub async fn refresh_remote(&self, name: &str) -> Result<(), String> {
        ...
        let index = match remote.fetch_index().await {
            ...
            Err(e) => {
                ...
                eprintln!("[forge] Warning: Cannot reach remote registry: {}", e);
        ...
        let toolchain = match index.toolchains.get(name) {
            Some(tc) => tc,
            None => {
                eprintln!("[forge] Warning: Runtime '{}' not found in registry index", name);
                return Ok(());
            }
        };
        ...
        match remote.fetch_metadata(name, latest).await {
            ...
            Err(e) => {
                ...
                if remote.load_cached(name, latest).is_some() {
                    eprintln!(
                        "[forge] Warning: Using stale cache for {}/{}: {}",
                        name, latest, e
                    );
                    return Ok(());
                }
    ...
    pub fn resolve(
        ...
        // ── Tier 2: local FRRS cache directory ──
    ...
    /// Scan the FRRS cache directory for `name`, find any versions
    ...
    fn resolve_from_cache(
        ...
        for entry in dir_entries {
            ...
            let metadata: FrrsMetadata = toml::from_str(&content).ok()?;
...
mod tests {
    ...
    // ── Phase 6.1: FRRS type deserialization ─────────────────
...
"#;
        ...
        let metadata: FrrsMetadata = toml::from_str(toml_str).expect("should parse");
...
"#;
        ...
        let metadata: FrrsMetadata = toml::from_str(toml_str).expect("should ignore unknown fields");
...
"#;
    ...
    fn test_load_cached_missing_file_returns_none() {
        let tmp = std::env::temp_dir().join("forge_test_cache_missing");
        let _ = fs::remove_dir_all(&tmp);
        let registry = RemoteRegistry::new("https://registry.forge.sh", tmp.clone());
    ...
    fn test_load_cached_fresh_file_returns_metadata() {
        let tmp = std::env::temp_dir().join("forge_test_cache_fresh");
        let _ = fs::remove_dir_all(&tmp);

        let meta = FrrsMetadata {
            name: "test".to_string(),
            version: "1.0.0".to_string(),
            license: None,
            homepage: None,
            artifacts: vec![FrrsArtifact {
                platform: "linux".to_string(),
                arch: "x86_64".to_string(),
