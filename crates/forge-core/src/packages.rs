...
pub fn install_pip_deps(
    ...
    // Resolve the forge-managed python binary from cache
...
/// Find the forge-managed python binary in the cache directory.
///
...
fn resolve_python_binary(cache_dir: &Path) -> Result<PathBuf, String> {
    ...
    if !python_dir.exists() {
        return Err(
            "No python runtime found in forge cache. Run 'forge up' to sync runtimes first."
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn test_install_pip_deps_no_packages_config() {
        ...
        let temp_dir = std::env::temp_dir().join("forge_pkgs_test_noop");
    ...
    fn test_install_pip_deps_missing_python() {
        ...
        let temp_dir = std::env::temp_dir().join("forge_pkgs_test_no_python");
        let workspace = temp_dir.join("workspace");
        let cache = temp_dir.join("cache");
        std::fs::create_dir_all(&workspace).unwrap();
        std::fs::create_dir_all(&cache).unwrap();
    ...
    fn test_install_pip_deps_missing_requirements() {
        ...
        let temp_dir = std::env::temp_dir().join("forge_pkgs_test_missing_reqs");
        let workspace = temp_dir.join("workspace");
        let cache = temp_dir.join("cache");
...
"#;
    ...
    #[test]
    fn test_packages_config_embedded_in_forge_config() {
        let toml_str = r#"
[runtimes]
python = "3.12.0"
...
"#;
        let config: crate::manifest::ForgeConfig = toml::from_str(toml_str).unwrap();
    ...
    #[test]
    fn test_forge_config_no_packages() {
...
"#;
    ...
    fn test_resolve_python_binary_empty_cache() {
        let temp_dir = std::env::temp_dir().join("forge_pkgs_resolve_empty");
        let cache = temp_dir.join("cache");
        std::fs::create_dir_all(&cache).unwrap();
    ...
    fn test_resolve_python_binary_finds_python() {
        let temp_dir = std::env::temp_dir().join("forge_pkgs_resolve_find");
        let cache = temp_dir.join("cache");
