    ...
    for (key, def) in definitions {
        ...
        if def.required && value_opt.is_none() {
            issues.push(DoctorIssue {
                ...
                remediation: format!("Define '{}' in forge.env, forge.local.toml, system environment, or secrets.", key),
            });
            continue;
        }
                ...
                match Regex::new(pattern_str) {
                    ...
                    Err(e) => {
                        issues.push(DoctorIssue {
                            ...
                            remediation: "Correct the regex pattern in the forge.toml config definitions.".to_string(),
                        });
                    }
                }
            }
...
/// 2. System environment overrides (FORGE_VAR_<KEY>)
/// 3. Local Developer Overrides (forge.local.toml)
/// 4. **Plugin Configuration Providers** (new)
/// 5. Secrets Providers (forge.secrets)
/// 6. Environment File (forge.env)
/// 7. Profile Overlays
/// 8. Defaults / Manifest
pub fn resolve_environment_with_plugins(
    ...
    let secrets_path = ctx.workspace_root().join("forge.secrets");
                ...
                for (key, secret_cfg) in secrets_manifest.secrets {
                    let resolved_secret = if secret_cfg.provider == "keyring" {
                        ...
                        let provider = crate::secrets::FallbackSecretProvider::new(
                            &workspace_id,
                            ctx.workspace_root().join(".forge").join("secrets.enc")
                        );
                        let lookup_key = secret_cfg.key.as_deref().or(secret_cfg.secret_id.as_deref()).unwrap_or(&key);
                        provider.get(lookup_key).ok().flatten()
                    } else {
    ...
    // Level 3: Local Developer Overrides (forge.local.toml)
    let local_path = ctx.workspace_root().join("forge.local.toml");
    ...
    // Level 2: System Environment Overrides (FORGE_VAR_<KEY>)
    for (k, v) in std::env::vars() {
        if k.starts_with("FORGE_VAR_") {
            let var_key = &k["FORGE_VAR_".len()..];
...
mod resolver_tests {
    ...
    fn test_plugin_config_provider_precedence() {
        ...
        let ctx = MockRuntimeContextProvider {
            workspace_root: PathBuf::from("/tmp/forge_test_plugin_config"),
