...
async fn test_download_sha_mismatch_and_deletion() {
    ...
    let temp_dir = std::env::temp_dir().join("anvil_test_cache_integration");
...
async fn test_parallel_download_and_abort() {
    let (url, _server_handle) = start_mock_server().await;
    let temp_dir = std::env::temp_dir().join("anvil_parallel_test_integration");
    fs::create_dir_all(&temp_dir).unwrap();
...
async fn test_sync_idempotency_skipped() {
    let temp_dir = std::env::temp_dir().join("anvil_sync_idempotency_test");
    ...
    let sha = anvil_core::installer::compute_sha256(&src_archive).unwrap();

    // 2. Set up workspace with anvil.toml and anvil.lock
    fs::write(
    ...
    fs::write(workspace_root.join("anvil.lock"), lock_content).unwrap();

    // 3. Run SyncOperation first time -> should install (Success)
    let event_bus = anvil_core::event_bus::EventBus::new(10);
    let mut ctx = anvil_core::operations::Context::new(workspace_root.clone(), cache_dir.clone(), event_bus);
    
    let sync_op = anvil_core::operations::SyncOperation;
    let plan1 = sync_op.plan(&ctx).unwrap();
    let res1 = sync_op.execute(&mut ctx, plan1).await.unwrap();
    assert_eq!(res1.status, anvil_core::types::OperationStatus::Success);
    ...
    assert_eq!(res2.status, anvil_core::types::OperationStatus::Skipped);
...
async fn test_e2e_lifecycle_state_transitions() {
    let temp_dir = std::env::temp_dir().join("anvil_e2e_transitions_test");
    let _ = fs::remove_dir_all(&temp_dir);
    fs::create_dir_all(&temp_dir).unwrap();
    ...
    let get_state = |wr: &Path, cd: &Path| -> anvil_core::types::LifecycleState {
        let toml_path = wr.join("anvil.toml");
        if !toml_path.exists() {
            return anvil_core::types::LifecycleState::Uninitialized;
        }
        let config = match anvil_core::manifest::load_config(&toml_path) {
            Ok(c) => c,
            Err(_) => return anvil_core::types::LifecycleState::Broken,
        };
        let lock_path = wr.join("anvil.lock");
        if !lock_path.exists() {
            return anvil_core::types::LifecycleState::Initialized;
        }
        let lockfile = match anvil_core::load_lockfile(&lock_path) {
            Ok(l) => l,
            Err(_) => return anvil_core::types::LifecycleState::Broken,
        };
        ...
        for runtime in &lockfile.runtimes {
            if config_runtimes.remove(&runtime.name).is_none() {
                return anvil_core::types::LifecycleState::Outdated;
            }
        }
        ...
        if any_missing {
            return anvil_core::types::LifecycleState::Locked;
        }
        let shims_cache = wr.join(".anvil").join("shims.cache");
        if !shims_cache.exists() {
            return anvil_core::types::LifecycleState::Synced;
        }
        anvil_core::types::LifecycleState::Ready
    };

    // State 1: Uninitialized (no anvil.toml)
    assert_eq!(get_state(&workspace_root, &cache_dir), anvil_core::types::LifecycleState::Uninitialized);

    // State 2: Initialized (run InitOperation)
    let mut ctx = anvil_core::operations::Context::new(workspace_root.clone(), cache_dir.clone(), event_bus.clone());
    let init_op = anvil_core::operations::InitOperation;
    let init_plan = init_op.plan(&ctx).unwrap();
    init_op.execute(&mut ctx, init_plan).await.unwrap();
    assert_eq!(get_state(&workspace_root, &cache_dir), anvil_core::types::LifecycleState::Initialized);
    ...
    fs::write(workspace_root.join("anvil.lock"), toml::to_string_pretty(&lockfile).unwrap()).unwrap();

    assert_eq!(get_state(&workspace_root, &cache_dir), anvil_core::types::LifecycleState::Locked);
    ...
    assert_eq!(get_state(&workspace_root, &cache_dir), anvil_core::types::LifecycleState::Ready);
...
async fn test_explain_resolution() {
    let temp_dir = std::env::temp_dir().join("anvil_test_explain_resolution");
    let _ = fs::remove_dir_all(&temp_dir);
    fs::create_dir_all(&temp_dir).unwrap();
    ...
    let engine = anvil_core::Engine::new(workspace_root.clone()).unwrap();
    let res = engine.explain("node").await.unwrap();
    assert_eq!(res.runtime, "node");
    assert_eq!(res.state, "Initialized"); // anvil.lock is missing
...
async fn test_trace_ascii_formatting() {
    let temp_dir = std::env::temp_dir().join("anvil_test_trace_ascii");
    let _ = fs::remove_dir_all(&temp_dir);
    fs::create_dir_all(&temp_dir).unwrap();
    
    let journal_file = temp_dir.join(".anvil").join("journal.jsonl");
    let event_bus = anvil_core::event_bus::EventBus::new_with_journal(10, journal_file);

    let op_id = "op-trace-123";
    
    // Publish mock events
    event_bus.publish(anvil_core::types::Event {
        timestamp: "2026-07-01T09:00:00.000-05:00".to_string(),
        operation_id: op_id.to_string(),
        runtime: "all".to_string(),
        phase: "Inspect".to_string(),
        status: anvil_core::types::EventStatus::Started,
        message: None,
        ..Default::default()
    }).unwrap();
    ...
    event_bus.publish(anvil_core::types::Event {
        ...
        runtime: "all".to_string(),
        phase: "Inspect".to_string(),
        status: anvil_core::types::EventStatus::Finished,
        message: None,
        ..Default::default()
    }).unwrap();
    ...
    let engine = anvil_core::Engine::new(temp_dir.clone()).unwrap();
...
async fn test_events_live_tailing() {
    let temp_dir = std::env::temp_dir().join("anvil_test_live_tailing");
    let _ = fs::remove_dir_all(&temp_dir);
    fs::create_dir_all(&temp_dir).unwrap();
    ...
    let event = anvil_core::types::Event {
        timestamp: "2026-07-01T09:00:00.000-05:00".to_string(),
...
async fn test_e2e_env_and_secrets() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("anvil_test_e2e_env_secrets_{}", rand::thread_rng().gen::<u32>()));
    let _ = fs::remove_dir_all(&temp_dir);
    fs::create_dir_all(&temp_dir).unwrap();

    std::env::set_var("ANVIL_MASTER_KEY", "integration-test-master-key");

    let anvil_toml_content = r#"
...
"#;
    fs::write(temp_dir.join("anvil.toml"), anvil_toml_content).unwrap();

    let anvil_secrets_content = r#"
[secrets]
DB_PASS = { provider = "file" }
"#;
    fs::write(temp_dir.join("anvil.secrets"), anvil_secrets_content).unwrap();
    ...
    std::env::remove_var("ANVIL_MASTER_KEY");
    fs::remove_dir_all(&temp_dir).ok();
}
