...
/// Manages snapshot CRUD as directory-based flat files under `.anvil/snapshots/{name}/`.
...
impl SnapshotManager {
    ...
    ///
    /// The snapshots directory is `<workspace_root>/.anvil/snapshots/`.
    pub fn new(workspace_root: &Path, cache_dir: &Path) -> Self {
        Self {
            workspace_root: workspace_root.to_path_buf(),
            snapshots_dir: workspace_root.join(".anvil").join("snapshots"),
    ...
    ///
    /// Copies `anvil.toml`, `anvil.lock`, captures `state.json` via
    /// [`crate::state::compute_current_state`], captures the last 100
    /// journal lines from `.anvil/journal.jsonl`, and writes a
    ...
    pub fn create(&self, name: Option<&str>, description: Option<&str>) -> Result<String, String> {
        ...
        // --- Validate anvil.toml exists ---
        let toml_src = self.workspace_root.join("anvil.toml");
        if !toml_src.exists() {
            let _ = std::fs::remove_dir_all(&snapshot_dir);
            return Err("anvil.toml not found. Cannot create snapshot.".to_string());
        }

        // --- Copy anvil.toml (verbatim) ---
        let toml_dst = snapshot_dir.join("anvil.toml");
        std::fs::copy(&toml_src, &toml_dst)
            .map_err(|e| format!("Failed to copy anvil.toml: {}", e))?;

        // --- Copy anvil.lock (verbatim, if it exists) ---
        let lock_src = self.workspace_root.join("anvil.lock");
        let lock_dst = snapshot_dir.join("anvil.lock");
        if lock_src.exists() {
            std::fs::copy(&lock_src, &lock_dst)
                .map_err(|e| format!("Failed to copy anvil.lock: {}", e))?;
        }
        ...
        let journal_path = self.workspace_root.join(".anvil").join("journal.jsonl");
        ...
        // --- Count runtimes from anvil.toml ---
    ...
    ///
    /// Backs up current `anvil.toml` and `anvil.lock` to `.bak` files,
    ...
    pub fn restore(&self, name: &str, dry_run: bool) -> Result<(), String> {
        ...
        let snapshot_toml = snapshot_dir.join("anvil.toml");
        if !snapshot_toml.exists() {
            return Err(format!("Snapshot '{}' is missing anvil.toml", name));
        }

        let snapshot_lock = snapshot_dir.join("anvil.lock");
        let current_toml = self.workspace_root.join("anvil.toml");
        let current_lock = self.workspace_root.join("anvil.lock");

        if dry_run {
            println!("Would restore from snapshot '{}':", name);
            println!("  anvil.toml -> {}", current_toml.display());
            if snapshot_lock.exists() {
                println!("  anvil.lock -> {}", current_lock.display());
            }
            return Ok(());
        }

        // --- Backup current files ---
        let bak_toml = self.workspace_root.join("anvil.toml.bak");
        let bak_lock = self.workspace_root.join("anvil.lock.bak");

        if current_toml.exists() {
            std::fs::copy(&current_toml, &bak_toml)
                .map_err(|e| format!("Failed to backup anvil.toml: {}", e))?;
        }
        if current_lock.exists() {
            std::fs::copy(&current_lock, &bak_lock)
                .map_err(|e| format!("Failed to backup anvil.lock: {}", e))?;
        }

        // --- Restore from snapshot ---
        let restore_toml = || -> Result<(), String> {
            std::fs::copy(&snapshot_toml, &current_toml)
                .map_err(|e| format!("Failed to restore anvil.toml: {}", e))?;

            if snapshot_lock.exists() {
                std::fs::copy(&snapshot_lock, &current_lock)
                    .map_err(|e| format!("Failed to restore anvil.lock: {}", e))?;
            } else if current_lock.exists() {
                std::fs::remove_file(&current_lock)
                    .map_err(|e| format!("Failed to remove stale anvil.lock: {}", e))?;
            }
            Ok(())
        };
...
mod tests {
    ...
    fn init_workspace(ws: &Path, toml_content: &str, lock_content: &str) {
        ...
        std::fs::write(ws.join("anvil.toml"), toml_content).unwrap();
        if !lock_content.is_empty() {
            std::fs::write(ws.join("anvil.lock"), lock_content).unwrap();
        }
        // Write some journal events
        let journal_content: String = (0..50)
            .map(|i| format!("{{\"event\": {}, \"ts\": \"2025-01-01T00:00:00Z\"}}\n", i))
            .collect();
        std::fs::write(ws.join(".anvil").join("journal.jsonl"), &journal_content).unwrap();
    }

    fn create_manager(ws: &Path) -> SnapshotManager {
        let cache_dir = ws.join(".anvil").join("cache");
    ...
    fn test_create_snapshot_creates_all_files() {
        ...
        let snap_dir = ws.join(".anvil").join("snapshots").join(&name);

        // Verify all 5 files exist
        assert!(snap_dir.join("anvil.toml").exists(), "anvil.toml should exist");
        assert!(snap_dir.join("anvil.lock").exists(), "anvil.lock should exist");
        ...
        // Verify anvil.toml is byte-identical
        let original_toml = std::fs::read(ws.join("anvil.toml")).unwrap();
        let snap_toml = std::fs::read(snap_dir.join("anvil.toml")).unwrap();
        assert_eq!(original_toml, snap_toml, "anvil.toml should be byte-identical");

        // Verify anvil.lock is byte-identical
        let original_lock = std::fs::read(ws.join("anvil.lock")).unwrap();
        let snap_lock = std::fs::read(snap_dir.join("anvil.lock")).unwrap();
        assert_eq!(original_lock, snap_lock, "anvil.lock should be byte-identical");
    }
    ...
    fn test_list_snapshots_multiple_sorted() {
        ...
        assert_eq!(alpha.anvil_version, env!("CARGO_PKG_VERSION"));
    }
    ...
    fn test_restore_dry_run_does_not_modify_files() {
        ...
        std::fs::write(ws.join("anvil.toml"), "[runtimes]\nmodified = \"*\"\n").unwrap();
        std::fs::write(ws.join("anvil.lock"), "[modified]\nversion = \"2.0\"\n").unwrap();

        // Dry-run should not modify files
        manager.restore("pre-upgrade", true).unwrap();

        // Files should still have modified content
        let toml_content = std::fs::read_to_string(ws.join("anvil.toml")).unwrap();
        assert!(toml_content.contains("modified"), "anvil.toml should NOT be restored in dry-run");
    }
    ...
    fn test_restore_replaces_files_correctly() {
        ...
        std::fs::write(ws.join("anvil.toml"), modified_toml).unwrap();
        std::fs::write(ws.join("anvil.lock"), modified_lock).unwrap();

        // Restore
        manager.restore("pre-upgrade", false).unwrap();

        // Files should be back to original
        let restored_toml = std::fs::read_to_string(ws.join("anvil.toml")).unwrap();
        let restored_lock = std::fs::read_to_string(ws.join("anvil.lock")).unwrap();
        assert_eq!(restored_toml, original_toml, "anvil.toml should be restored");
        assert_eq!(restored_lock, original_lock, "anvil.lock should be restored");
    }
    ...
    #[test]
    fn test_restore_without_anvil_toml_in_snapshot() {
        let (_ctx, ws) = TestContext::new();
        init_workspace(&ws, "[runtimes]\nnode = \">=18\"\n", "");
        let manager = create_manager(&ws);

        // Create a snapshot then remove its anvil.toml to simulate corruption
    ...
    fn test_restore_bak_files_created() {
        ...
        // Modify current files
        std::fs::write(ws.join("anvil.toml"), "modified").unwrap();
        std::fs::write(ws.join("anvil.lock"), "modified").unwrap();

        // Restore
        manager.restore("pre-upgrade", false).unwrap();

        // Bak files should exist and contain the modified content
        let bak_toml = std::fs::read_to_string(ws.join("anvil.toml.bak")).unwrap();
        let bak_lock = std::fs::read_to_string(ws.join("anvil.lock.bak")).unwrap();
