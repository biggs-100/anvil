...
pub fn get_cache_dir() -> Result<PathBuf, String> {
    ...
    let new_dir = home.join(".anvil").join("runtimes");
    let old_dir = home.join(".forge").join("runtimes");

    // Migrate old forge cache if it exists and new one doesn't
    if old_dir.exists() && !new_dir.exists() {
        if let Some(parent) = new_dir.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        match std::fs::rename(&old_dir, &new_dir) {
            Ok(_) => eprintln!("info: migrated cache from ~/.forge/runtimes to ~/.anvil/runtimes"),
            Err(e) => eprintln!("warning: could not migrate old cache: {}", e),
        }
    }
...
pub fn write_shims_cache_file(cache_file_path: &Path, map: &HashMap<String, PathBuf>) -> Result<(), String> {
    ...
    content.push_str("# anvil-shims-cache-v1\n");
...
pub fn regenerate_shims_cache(lockfile: &Lockfile, cache_dir: &Path, workspace_dir: &Path) -> Result<(), String> {
    let cache_file_path = workspace_dir.join(".anvil").join("shims.cache");
...
pub fn append_to_gitignore(workspace_dir: &Path) -> Result<(), String> {
    ...
    let entries = [".anvil/shims.cache", ".anvil/state.json"];
...
mod tests {
    ...
    fn test_append_to_gitignore() {
        let temp_dir = std::env::temp_dir().join("anvil_gitignore_test");
        ...
        assert!(content.contains(".anvil/shims.cache"));
        assert!(content.contains(".anvil/state.json"));
    ...
    fn test_shims_cache_serialization() {
        let temp_dir = std::env::temp_dir().join("anvil_core_shims_cache_test");
        fs::create_dir_all(&temp_dir).unwrap();
