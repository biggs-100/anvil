...
pub fn find_anvil_env(start_dir: &Path) -> Option<PathBuf> {
    let mut current = start_dir.to_path_buf();
    loop {
        let candidate = current.join("anvil.env");
        if candidate.exists() {
...
mod tests {
    ...
    fn test_mask_env_vars() {
        ...
        env.insert("DB_USER".to_string(), "anvil".to_string());
        
        let masked = mask_env_vars(&env);
        assert_eq!(masked.get("API_KEY").unwrap(), "[REDACTED]");
        assert_eq!(masked.get("DB_USER").unwrap(), "anvil");
    }
}
