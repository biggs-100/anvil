...
/// Policy configuration parsed from the `[policy]` section in `anvil.toml`.
///
...
impl PolicyEngine {
    ...
    /// Check policy rules that apply to `anvil up`:
    ///
    ...
    pub fn check_before_up(
        ...
        if !self.config.allow_network {
            violations.push(PolicyViolation {
                ...
                expected: "true".into(),
                current: "false".into(),
                message: "Network access is prohibited but 'anvil up' requires it".into(),
            });
        }
    ...
    /// Check policy rules that apply to `anvil sync`:
    ///
    /// - `allow_network`
    /// - `require_hashes`
    pub fn check_before_sync(
        ...
        if !self.config.allow_network {
            violations.push(PolicyViolation {
                ...
                expected: "true".into(),
                current: "false".into(),
                message: "Network access is prohibited but 'anvil sync' requires it".into(),
            });
        }
    ...
    /// Check policy rules that apply to `anvil run` / `anvil shell`:
    ///
    /// - `minimum_health`
    /// - `required_profiles`
    /// - `forbid_runtimes`
