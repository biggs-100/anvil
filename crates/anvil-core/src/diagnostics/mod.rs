...
impl crate::environment::RuntimeContextProvider for DiagnosticContext {
    ...
    fn runtime_path(&self, name: &str) -> Option<PathBuf> {
        let lockfile_path = self.workspace_root.join("anvil.lock");
...
impl HealthCheck for ManifestCheck {
    ...
    fn description(&self) -> &'static str { "Validates existence and syntax of anvil.toml" }
        ...
        async move {
            ...
            if !toml_path.exists() {
                return Ok(vec![Finding {
                    ...
                    confidence: 100,
                    message: "Missing anvil.toml manifest file".to_string(),
                    explanation: Explanation {
                        what: "The project manifest file (anvil.toml) is missing".to_string(),
                        why: "Anvil requires a anvil.toml file in the workspace root to define managed runtimes".to_string(),
                        how: "Run 'anvil init' to initialize a new anvil.toml manifest".to_string(),
                    },
                    suggested_quick_fix: Some(QuickFix {
                        description: "Initialize anvil.toml".to_string(),
                        action: QuickFixAction::RegenerateLockfile,
                    }),
                    doc_url: Some("https://anvil.dev/docs/manifest".to_string()),
                }]);
            }
            match crate::manifest::load_config(&toml_path) {
                ...
                Err(err) => Ok(vec![Finding {
                    ...
                    severity: Severity::ERROR,
                    confidence: 100,
                    message: format!("Syntax/deserialization errors in anvil.toml: {}", err),
                    explanation: Explanation {
                        what: "Failed to parse the anvil.toml configuration".to_string(),
                        why: "The manifest file contains invalid TOML syntax or structural errors".to_string(),
                        how: "Fix the syntax errors in anvil.toml".to_string(),
                    },
                    suggested_quick_fix: None,
                    doc_url: Some("https://anvil.dev/docs/manifest".to_string()),
                }]),
...
impl HealthCheck for LockCheck {
    ...
    fn description(&self) -> &'static str { "Validates existence and sync status of anvil.lock" }
    fn category(&self) -> &'static str { "lock" }
    fn dependencies(&self) -> Vec<&'static str> { vec!["FG001"] }
    fn check<'a>(&'a self, ctx: &'a DiagnosticContext) -> BoxFuture<'a, Result<Vec<Finding>, String>> {
        async move {
            let lock_path = ctx.workspace_root.join("anvil.lock");
            if !lock_path.exists() {
                return Ok(vec![Finding {
                    ...
                    severity: Severity::ERROR,
                    confidence: 100,
                    message: "Missing anvil.lock lockfile".to_string(),
                    explanation: Explanation {
                        what: "The dependency lockfile (anvil.lock) is missing".to_string(),
                        why: "Anvil uses the lockfile to ensure reproducible runtime installations".to_string(),
                        how: "Run 'anvil lock' or 'anvil install' to generate anvil.lock".to_string(),
                    },
                    suggested_quick_fix: Some(QuickFix {
                        description: "Regenerate the lockfile".to_string(),
                        action: QuickFixAction::RegenerateLockfile,
                    }),
                    doc_url: Some("https://anvil.dev/docs/lockfile".to_string()),
                }]);
            }
            let toml_path = ctx.workspace_root.join("anvil.toml");
            ...
            let lockfile = match crate::lock::load_lockfile(&lock_path) {
                ...
                Err(err) => {
                    return Ok(vec![Finding {
                        ...
                        severity: Severity::ERROR,
                        confidence: 100,
                        message: format!("Failed to parse anvil.lock: {}", err),
                        explanation: Explanation {
                            what: "The lockfile is corrupted".to_string(),
                            why: "The file anvil.lock contains invalid structure".to_string(),
            ...
            if out_of_sync {
                Ok(vec![Finding {
                    ...
                    explanation: Explanation {
                        what: "Dependencies defined in anvil.toml do not match anvil.lock".to_string(),
                        why: "A runtime was added, removed, or changed in anvil.toml without updating the lockfile".to_string(),
                        how: "Run 'anvil install' or 'anvil lock' to synchronize dependencies".to_string(),
                    },
                    suggested_quick_fix: Some(QuickFix {
                        description: "Regenerate the lockfile".to_string(),
                        action: QuickFixAction::RegenerateLockfile,
                ...
                if !folder_healthy {
                    findings.push(Finding {
                        ...
                        explanation: Explanation {
                            ...
                            how: "Run 'anvil install' to re-extract the runtime".to_string(),
                        },
                        suggested_quick_fix: Some(QuickFix {
                        ...
                        if computed_hash != runtime.sha256 {
                            findings.push(Finding {
                                ...
                                explanation: Explanation {
                                    ...
                                    how: "Delete the archive and re-download by running anvil install".to_string(),
                                },
                                suggested_quick_fix: Some(QuickFix {
                                    description: format!("Wipe and re-extract {}", runtime.name),
                                    action: QuickFixAction::WipeAndReextract {
...
impl HealthCheck for SecretCheck {
    ...
    fn check<'a>(&'a self, ctx: &'a DiagnosticContext) -> BoxFuture<'a, Result<Vec<Finding>, String>> {
        async move {
            let mut findings = Vec::new();
            let secrets_path = ctx.workspace_root.join(".anvil").join("secrets.json");
                ...
                if payload.is_err() {
                    findings.push(Finding {
                        ...
                        suggested_quick_fix: Some(QuickFix {
                            ...
                            action: QuickFixAction::AddToGitIgnore { path: ".anvil/secrets.json".to_string() },
                        }),
                        doc_url: None,
                    });
                } else if std::env::var("ANVIL_MASTER_KEY").is_err() {
                    findings.push(Finding {
                        code: "FG008".to_string(),
                        category: self.category().to_string(),
                        severity: Severity::ERROR,
                        confidence: 100,
                        message: "ANVIL_MASTER_KEY master password env variable is not configured".to_string(),
                        explanation: Explanation {
                            what: "Master key is missing".to_string(),
                            why: "Decryption of secrets.json requires the master passphrase in ANVIL_MASTER_KEY".to_string(),
                            how: "Define ANVIL_MASTER_KEY in your system environment".to_string(),
                        },
                        suggested_quick_fix: Some(QuickFix {
                            description: "Set secret ANVIL_MASTER_KEY".to_string(),
                            action: QuickFixAction::SetSecret { key: "ANVIL_MASTER_KEY".to_string() },
                        }),
                        doc_url: None,
                    });
                }
                ...
                Err(err) => {
                    findings.push(Finding {
                        ...
                        explanation: Explanation {
                            ...
                            how: "Verify anvil.toml profiles and variables".to_string(),
                        },
                        suggested_quick_fix: None,
                        doc_url: None,
                    });
...
impl HealthCheck for PathCheck {
    ...
    fn description(&self) -> &'static str { "Checks if the global Anvil shim folder is in the system PATH" }
        ...
        async move {
            ...
            if !in_path {
                Ok(vec![Finding {
                    ...
                    severity: Severity::WARNING,
                    confidence: 100,
                    message: format!("Shim directory ({:?}) is not present in the system's PATH", anvil_bin),
                    explanation: Explanation {
                        what: "Anvil shim folder is missing from system PATH".to_string(),
                        why: "To run runtimes natively from terminal, your shell must look inside ~/.anvil/bin".to_string(),
                        how: "Append ~/.anvil/bin to your PATH environment variable in your shell profile".to_string(),
                    },
                    suggested_quick_fix: None,
                    doc_url: Some("https://anvil.dev/docs/installation#path".to_string()),
                }])
            } else {
                Ok(Vec::new())
            }
...
impl HealthCheck for ShimCheck {
    ...
    fn check<'a>(&'a self, ctx: &'a DiagnosticContext) -> BoxFuture<'a, Result<Vec<Finding>, String>> {
        async move {
            ...
            let cache_file_path = ctx.workspace_root.join(".anvil").join("shims.cache");
            if !cache_file_path.exists() {
                return Ok(vec![Finding {
                    ...
                    explanation: Explanation {
                        ...
                        how: "Run 'anvil shims setup' or trigger repair".to_string(),
                    },
                    suggested_quick_fix: Some(QuickFix {
        ...
        async move {
            ...
            Ok(vec![Finding {
                ...
                explanation: Explanation {
                    ...
                    how: "To free space, run 'anvil cache clean'".to_string(),
                },
                suggested_quick_fix: None,
                doc_url: None,
            }])
...
impl HealthCheck for ProfileCheck {
    ...
    fn description(&self) -> &'static str { "Validates active environment profile configuration inside anvil.toml" }
            ...
            if let Some(ref active_profile) = ctx.active_profile {
                ...
                if !profile_exists {
                    findings.push(Finding {
                        ...
                        severity: Severity::ERROR,
                        confidence: 100,
                        message: format!("Active profile '{}' is not defined in anvil.toml", active_profile),
                        explanation: Explanation {
                            ...
                            how: "Define the profile section in anvil.toml or change the active profile".to_string(),
                        },
                        suggested_quick_fix: None,
                        doc_url: None,
                    });
...
mod tests {
    ...
    async fn test_dag_scheduler_short_circuit() {
        let temp_dir = std::env::temp_dir().join("anvil_diag_test_dag");
