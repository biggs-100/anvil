...
impl Engine {
    ...
    pub async fn explain(&self, runtime: &str) -> Result<RuntimeExplanation, String> {
        ...
        let toml_path = self.workspace_root.join("anvil.toml");
        if !toml_path.exists() {
            return Err("No anvil.toml manifest found".to_string());
        }
        let config = crate::manifest::load_config(&toml_path)?;
        let version_req = match config.runtimes.get(runtime) {
            Some(v) => v,
            None => return Err(format!("Runtime '{}' is not configured in anvil.toml", runtime)),
        };

        diagnostics.push(format!("Configured version requirement: {}", version_req.version()));

        let lock_path = self.workspace_root.join("anvil.lock");
        if !lock_path.exists() {
            return Ok(RuntimeExplanation {
                runtime: runtime.to_string(),
                state: "Initialized".to_string(),
                diagnostics: vec!["anvil.lock is missing".to_string()],
            });
        }
        ...
        let lock_entry = match lock_entry {
            Some(e) => e,
            None => {
                diagnostics.push("Not present in anvil.lock".to_string());
                return Ok(RuntimeExplanation {
                    runtime: runtime.to_string(),
        ...
        let state = if is_extracted {
            ...
            let shims_cache = self.workspace_root.join(".anvil").join("shims.cache");
    ...
    pub async fn env_list(&self) -> Result<HashMap<String, String>, String> {
        let env_path = crate::environment::find_anvil_env(&self.workspace_root);
    ...
    pub async fn env_set(&self, key: &str, value: &str) -> Result<(), String> {
        let env_path = crate::environment::find_anvil_env(&self.workspace_root)
            .unwrap_or_else(|| self.workspace_root.join("anvil.env"));
        ...
        std::fs::write(&env_path, lines.join("\n"))
            .map_err(|e| format!("Failed to write anvil.env: {}", e))?;
        Ok(())
    }
    ...
    pub async fn secret_set(&self, key: &str, value: &str) -> Result<(), String> {
        ...
        let secrets_manifest_path = self.workspace_root.join("anvil.secrets");
        ...
        let mut manifest = if secrets_manifest_path.exists() {
            let content = std::fs::read_to_string(&secrets_manifest_path)
                .map_err(|e| format!("Failed to read anvil.secrets: {}", e))?;
            toml::from_str::<SecretsManifest>(&content)
                .map_err(|e| format!("Failed to parse anvil.secrets: {}", e))?
        } else {
        ...
        if let Some(cfg) = manifest.secrets.get(key) {
            ...
            let test_entry = keyring::Entry::new("anvil-secrets", &format!("{}::_test_conn", workspace_id));
            ...
            let toml_content = toml::to_string_pretty(&manifest)
                .map_err(|e| format!("Failed to serialize anvil.secrets: {}", e))?;
            std::fs::write(&secrets_manifest_path, toml_content)
                .map_err(|e| format!("Failed to write anvil.secrets: {}", e))?;
        }
        ...
        if provider_name == "keyring" {
            ...
            let provider = crate::secrets::FallbackSecretProvider::new(
                &workspace_id,
                self.workspace_root.join(".anvil").join("secrets.enc")
            );
            provider.set(key, value)?;
        }
    ...
    pub async fn secret_doctor(&self) -> Result<Vec<String>, String> {
        ...
        let enc_file = self.workspace_root.join(".anvil").join("secrets.enc");
        if enc_file.exists() {
            ...
            if let Ok(_phrase) = crate::secrets::get_passphrase() {
                ...
                report.push("Fallback store decryption: Passphrase not configured (ANVIL_MASTER_KEY not set)".to_string());
            }
        } else {
...
mod tests {
    ...
    fn test_engine_register_plugin() {
        ...
        let temp_dir = std::env::temp_dir().join("anvil_plugin_engine_test");
    ...
    fn test_new_with_plugins() {
        ...
        let temp_dir = std::env::temp_dir().join("anvil_plugin_factory_test");
        let _ = std::fs::create_dir_all(&temp_dir);
