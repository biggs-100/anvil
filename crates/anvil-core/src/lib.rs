...
pub use context::{
    ...
    AnvilContext, AcpHandshake, AcpHandshakeRequest, AcpHandshakeResponse,
...
pub use plugin::{
    Plugin, CliCommand, PluginRegistry, PluginError, ExtensionSink, ANVIL_PLUGIN_API_VERSION,
};
...
pub use manifest::{AnvilConfig, RuntimeEntry, PackagesConfig, find_anvil_toml, load_config};
...
pub use registry::{
    HybridRegistry, RegistryEntry, ArrsMetadata, ArrsArtifact, RegistryIndex, RegistryIndexEntry,
    RemoteRegistry,
...
pub use environment::{find_anvil_env, parse_env_file, is_secret, mask_env_vars, RuntimeContextProvider, materialize_environment};
...
/// Orchestrates updating the lockfile from anvil.toml config.
///
/// The resolution chain:
///   1. Flat entries loaded from `metadata_cache.toml` (legacy)
///   2. ARRS cache directory (`.anvil/metadata_cache/`)
///   3. Remote ARRS registry (HTTP, best-effort pre-fetch)
///   4. Embedded compiled-in defaults
///
/// The remote registry URL is configured via `ANVIL_REGISTRY_URL` env var
/// (default: `https://registry.anvil.dev`). Set to empty string to disable
...
pub async fn update_lockfile(toml_path: &Path, lockfile_path: &Path) -> Result<Lockfile, String> {
    ...
    let registry_path = workspace_dir.join(".anvil").join("metadata_cache.toml");
    let frrs_cache_dir = workspace_dir.join(".anvil").join("metadata_cache");
    ...
    // Attach ARRS cache directory
    registry = registry.with_cache_dir(frrs_cache_dir.clone());

    // Configure remote registry via ANVIL_REGISTRY_URL env var
    ...
    for (name, runtime_entry) in &config.runtimes {
        // Best-effort pre-fetch from remote to populate ARRS cache
