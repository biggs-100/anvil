...
impl SecretProvider for KeyringSecretProvider {
    ...
    fn get(&self, key: &str) -> Result<Option<String>, String> {
        ...
        let entry = Entry::new("anvil-secrets", &entry_name)
    ...
    fn set(&self, key: &str, value: &str) -> Result<(), String> {
        ...
        if !keys.contains(&key.to_string()) {
            ...
            let list_entry = Entry::new("anvil-secrets", &list_entry_name)
                .map_err(|e| format!("Keyring error: {}", e))?;
...
pub fn get_passphrase() -> Result<String, String> {
    if let Ok(key) = std::env::var("ANVIL_MASTER_KEY") {
        return Ok(key);
    }
    #[cfg(test)]
    {
        return Err("No master key provided (ANVIL_MASTER_KEY not set in test environment)".to_string());
    }
    #[cfg(not(test))]
    {
        print!("Enter Anvil master passphrase: ");
        use std::io::Write;
...
mod tests {
    ...
    fn test_ci_bypass_via_env_var() {
        std::env::set_var("ANVIL_MASTER_KEY", "ci-secret-bypass");
