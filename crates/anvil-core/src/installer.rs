...
pub async fn install_runtime_transactional(
    ...
    let staging_dir = workspace_root.join(".anvil").join("staging").join(operation_id).join(&lock.name).join(&lock.version);
    ...
    let backup_dir = workspace_root.join(".anvil").join("backup").join(operation_id).join(&lock.name).join(&lock.version);
...
pub async fn install_runtimes(
    ...
    let workspace_root = crate::manifest::find_anvil_toml(&current_dir)
...
mod tests {
    ...
    async fn test_installer_successful_install() {
        let temp_dir = std::env::temp_dir().join("anvil_installer_success_test");
    ...
    async fn test_installer_hash_mismatch_rollback() {
        let temp_dir = std::env::temp_dir().join("anvil_installer_mismatch_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
        ...
        let staging_dir = workspace_root.join(".anvil").join("staging").join("op-test-mismatch").join("node").join("20.10.0");
        assert!(!staging_dir.exists());
    }

    #[tokio::test]
    async fn test_installer_validation_failure_rollback() {
        let temp_dir = std::env::temp_dir().join("anvil_installer_validation_test");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
        ...
        let staging_dir = workspace_root.join(".anvil").join("staging").join("op-test-validation").join("node").join("20.10.0");
        assert!(!staging_dir.exists());
    }
}
