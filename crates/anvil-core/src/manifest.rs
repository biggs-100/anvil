...
/// A runtime entry from `anvil.toml` — either a bare version string
...
pub fn find_anvil_toml(start_dir: &Path) -> Option<PathBuf> {
    let mut current = start_dir.to_path_buf();
    loop {
        let anvil_candidate = current.join("anvil.toml");
        if anvil_candidate.exists() {
            return Some(anvil_candidate);
        }
        // Fallback: detect old forge.toml and suggest migration
        let forge_candidate = current.join("forge.toml");
        if forge_candidate.exists() {
            eprintln!("info: found forge.toml — rename to anvil.toml to use with this version");
            return Some(forge_candidate);
        }
        if !current.pop() {
            break;
        }
...
pub fn load_config(toml_path: &Path) -> Result<AnvilConfig, String> {
    let content = fs::read_to_string(toml_path)
        .map_err(|e| format!("Failed to read anvil.toml (or forge.toml): {}", e))?;
...
python = "3.12.0"
"#;
        let config: AnvilConfig = toml::from_str(toml_str).unwrap();
...
"#;
    ...
    fn test_serde_roundtrip() {
        // Test roundtrip via AnvilConfig (TOML requires a table root)
        let config: AnvilConfig = toml::from_str(r#"
            [runtimes]
            node = "20.11.0"
        "#).unwrap();
        let node = config.runtimes.get("node").unwrap();
        ...
        let deserialized: AnvilConfig = toml::from_str(&serialized).unwrap();
        assert_eq!(
        ...
        let config2: AnvilConfig = toml::from_str(r#"
            [runtimes]
            node = { version = "20.11.0", sha256 = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" }
        "#).unwrap();
        ...
        let deserialized2: AnvilConfig = toml::from_str(&serialized2).unwrap();
        assert_eq!(
