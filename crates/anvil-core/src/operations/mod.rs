...
use crate::manifest::{AnvilConfig, load_config};
...
impl Context {
    ...
    pub fn load_config(&mut self) -> Result<&AnvilConfig, String> {
        if self.config.is_none() {
            let toml_path = if self.workspace_root.join("anvil.toml").exists() {
                self.workspace_root.join("anvil.toml")
            } else if self.workspace_root.join("forge.toml").exists() {
                self.workspace_root.join("forge.toml")
            } else {
                return Err("No anvil.toml or forge.toml manifest found".to_string());
            };
    ...
    pub fn load_lockfile(&mut self) -> Result<&Lockfile, String> {
        if self.lockfile.is_none() {
            let lock_path = if self.workspace_root.join("anvil.lock").exists() {
                self.workspace_root.join("anvil.lock")
            } else if self.workspace_root.join("forge.lock").exists() {
                self.workspace_root.join("forge.lock")
            } else {
                self.workspace_root.join("anvil.lock")
            };
...
impl Operation for InitOperation {
    ...
    fn plan(&self, _ctx: &Context) -> Result<Box<dyn Plan>, String> {
        Ok(Box::new(SimplePlan {
            name: "init".to_string(),
            details: json!({ "action": "Initialize anvil.toml and gitignore" }),
        }))
    }
    ...
    fn execute<'a>(&'a self, ctx: &'a mut Context, _plan: Box<dyn Plan>) -> OperationFuture<'a> {
        ...
        if !toml_path.exists() {
            std::fs::write(&toml_path, "[runtimes]\n")
                .map_err(|e| format!("Failed to create anvil.toml: {}", e))?;
        ...
        Ok(OperationResult {
            ...
            warnings: Vec::new(),
            changes,
            diagnostics: vec!["Anvil initialized successfully".to_string()],
        })
        })
    }
}
...
impl Operation for ResolveOperation {
    ...
    fn execute<'a>(&'a self, ctx: &'a mut Context, _plan: Box<dyn Plan>) -> OperationFuture<'a> {
        ...
        let registry_path = ctx.workspace_root.join(".anvil").join("metadata_cache.toml");
...
impl Operation for LockOperation {
    ...
    fn plan(&self, _ctx: &Context) -> Result<Box<dyn Plan>, String> {
        Ok(Box::new(SimplePlan {
            name: "lock".to_string(),
            details: json!({ "action": "Generate or update anvil.lock" }),
        }))
    }
    ...
    fn execute<'a>(&'a self, ctx: &'a mut Context, _plan: Box<dyn Plan>) -> OperationFuture<'a> {
        Box::pin(async move {
        let start = std::time::Instant::now();
        let toml_path = if ctx.workspace_root.join("anvil.toml").exists() {
            ctx.workspace_root.join("anvil.toml")
        } else if ctx.workspace_root.join("forge.toml").exists() {
            ctx.workspace_root.join("forge.toml")
        } else {
            ctx.workspace_root.join("anvil.toml")
        };
        let lockfile_path = ctx.workspace_root.join("anvil.lock");
        
        if !toml_path.exists() {
            return Err("anvil.toml (or forge.toml) not found".to_string());
        }
...
impl Operation for CleanOperation {
    ...
    fn execute<'a>(&'a self, ctx: &'a mut Context, _plan: Box<dyn Plan>) -> OperationFuture<'a> {
        ...
        let anvil_internal = ctx.workspace_root.join(".anvil");
        if anvil_internal.exists() {
            let path_str = anvil_internal.to_string_lossy().into_owned();
            std::fs::remove_dir_all(&anvil_internal)
                .map_err(|e| format!("Failed to clean .anvil directory: {}", e))?;
            changes.push(ChangeRecord {
                path: path_str,
                action: "deleted".to_string(),
            });
...
impl Operation for ValidateOperation {
    ...
    fn plan(&self, _ctx: &Context) -> Result<Box<dyn Plan>, String> {
        Ok(Box::new(SimplePlan {
            ...
            details: json!({ "action": "Validate anvil.toml and anvil.lock schemas" }),
        }))
    }
    ...
    fn execute<'a>(&'a self, ctx: &'a mut Context, _plan: Box<dyn Plan>) -> OperationFuture<'a> {
        ...
        if !toml_path.exists() {
            diagnostics.push("anvil.toml (or forge.toml) is missing".to_string());
            return Ok(OperationResult {
        ...
        match load_config(&toml_path) {
            Ok(config) => {
                diagnostics.push(format!("anvil.toml parsed correctly with {} runtimes", config.runtimes.len()));
            }
            Err(e) => {
                diagnostics.push(format!("anvil.toml validation error: {}", e));
                return Ok(OperationResult {
                    status: OperationStatus::Failure,
                    duration_ms: start.elapsed().as_millis() as u64,
                    warnings,
        ...
        let lock_path = if ctx.workspace_root.join("anvil.lock").exists() {
            ctx.workspace_root.join("anvil.lock")
        } else if ctx.workspace_root.join("forge.lock").exists() {
            ctx.workspace_root.join("forge.lock")
        } else {
            ctx.workspace_root.join("anvil.lock")
        };
        if lock_path.exists() {
            match load_lockfile(&lock_path) {
                Ok(lockfile) => {
                    diagnostics.push(format!("anvil.lock validated with {} locked runtimes", lockfile.runtimes.len()));
                }
                Err(e) => {
                    diagnostics.push(format!("anvil.lock validation error: {}", e));
                    return Ok(OperationResult {
                        status: OperationStatus::Failure,
                        duration_ms: start.elapsed().as_millis() as u64,
                        warnings,
            ...
            warnings.push("anvil.lock does not exist. Run 'anvil lock' to generate it".to_string());
        }
            ...
            match action {
                ...
                crate::diagnostics::QuickFixAction::SetEnvVar { key, value } => {
                    if let Some(env_path) = crate::environment::find_anvil_env(&ctx.workspace_root)
                        .or_else(|| Some(ctx.workspace_root.join("anvil.env")))
                    {
