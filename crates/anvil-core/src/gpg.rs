...
/// Embedded public key for registry.anvil.dev.
/// This is the trust anchor for verifying ARRS metadata signatures.
...
/// Parse `ANVIL_TRUSTED_KEYS` env var into a list of additional armored key strings.
...
pub fn parse_trusted_keys_env() -> Vec<String> {
    match std::env::var("ANVIL_TRUSTED_KEYS") {
            ...
            for part in raw.split(';') {
                ...
                for sub in part.split("\n\n") {
                    ...
                    if sub.contains("-----BEGIN PGP PUBLIC KEY BLOCK-----") {
                        ...
                        eprintln!(
                            "[anvil] Warning: ANVIL_TRUSTED_KEYS contains entry that does not \
...
/// plus any keys from `ANVIL_TRUSTED_KEYS`.
///
...
pub fn verify_gpg_signature(
    ...
    if !gpg_available {
        eprintln!(
            "[anvil] Warning: gpg not found on PATH — skipping signature verification. \
             Set ANVIL_GPG_STRICT=1 to require gpg."
        );
        if std::env::var("ANVIL_GPG_STRICT").as_deref() == Ok("1") {
            return Err("gpg not found and ANVIL_GPG_STRICT=1 is set".to_string());
        }
        return Ok("unverified".to_string());
    }

    // Build temp directory for keyring
    let tmp_dir = std::env::temp_dir().join(format!("anvil_gpg_{}", std::process::id()));
    ...
    if let Err(ref e) = import_result {
        eprintln!("[anvil] Warning: Failed to import embedded GPG key: {}", e);
    }

    // Import additional trusted keys
    for key_text in additional_keys {
        if let Err(e) = import_armored_key(&tmp_dir, key_text) {
            eprintln!("[anvil] Warning: Failed to import additional GPG key: {}", e);
        }
    }
...
mod tests {
    ...
    fn test_extract_key_id_from_line() {
        let line = "gpg: Good signature from \"Anvil Registry <registry@anvil.dev>\" [C0FFEE01]";
