...
use super::{CliCommand, ExtensionSink, Plugin, PluginError, ANVIL_PLUGIN_API_VERSION};
...
impl PluginRegistry {
    ...
    pub fn register(&mut self, plugin: Box<dyn Plugin>) -> Result<(), PluginError> {
        ...
        if api_version != ANVIL_PLUGIN_API_VERSION {
            return Err(PluginError::ApiVersionMismatch {
                plugin: name,
                expected: ANVIL_PLUGIN_API_VERSION.to_string(),
                actual: api_version,
            });
        }
...
mod tests {
    ...
    impl TestPlugin {
        fn new(name: &str) -> Self {
            Self {
                ...
                api_version: ANVIL_PLUGIN_API_VERSION.to_string(),
                register_result: Ok(()),
            }
        }
    ...
    fn test_register_plugin() {
        ...
        assert_eq!(plugin.api_version(), ANVIL_PLUGIN_API_VERSION);
    ...
    fn test_api_version_mismatch() {
        ...
        match err {
            PluginError::ApiVersionMismatch { plugin, expected, actual } => {
                ...
                assert_eq!(expected, ANVIL_PLUGIN_API_VERSION);
