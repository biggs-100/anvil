        ...
        tokio::spawn(async move {
            while let Ok(event) = rx.recv().await {
                let journal_path = if let Some(ref path) = custom_path {
                    path.clone()
                } else if let Ok(override_path) = std::env::var("ANVIL_JOURNAL_PATH") {
                    ...
                    workspace_root.join(".anvil").join("journal.jsonl")
                };
...
mod tests {
    ...
    async fn test_event_ndjson_serialization() {
        let temp_dir = std::env::temp_dir().join("anvil_test_event_bus");
    ...
    async fn test_concurrent_appends() {
        let temp_dir = std::env::temp_dir().join("anvil_test_event_bus_concurrent");
        let _ = std::fs::remove_dir_all(&temp_dir);
        std::fs::create_dir_all(&temp_dir).unwrap();
