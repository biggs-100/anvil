...
pub trait ContextExporter: Send + Sync {
    fn name(&self) -> &'static str;
    fn export(&self, context: &AnvilContext) -> Result<String, String>;
}

pub trait AgentAdapter: Send + Sync {
    fn name(&self) -> &'static str;
    fn adapt(&self, context: &AnvilContext, exporter: &dyn ContextExporter) -> Result<String, String>;
}
...
pub struct AcpHandshake {
    pub request: AcpHandshakeRequest,
}
...
impl ContextEngine {
    ...
    pub fn handshake(&self, req_json: &str) -> Result<String, String> {
        let req: AcpHandshakeRequest = serde_json::from_str(req_json)
    ...
    pub async fn query(&self, options: &ContextOptions) -> Result<AnvilContext, String> {
...
impl ContextProvider for RuntimeProviderImpl {
    ...
    fn collect(&self, options: &ContextOptions) -> Result<serde_json::Value, String> {
        let lockfile_path = options.workspace_root.join("anvil.lock");
...
impl ContextProvider for ConfigurationProviderImpl {
    ...
    fn collect(&self, options: &ContextOptions) -> Result<serde_json::Value, String> {
        let toml_path = options.workspace_root.join("anvil.toml");
        if toml_path.exists() {
...
impl ContextProvider for WorkspaceProviderImpl {
    ...
    fn collect(&self, options: &ContextOptions) -> Result<serde_json::Value, String> {
        ...
        while let Some(current) = stack.pop() {
            ...
            if file_name == ".git" || file_name == "node_modules" || file_name == "target" || file_name == ".anvil" {
                continue;
            }
...
impl ContextExporter for MarkdownExporter {
    ...
    fn export(&self, context: &AnvilContext) -> Result<String, String> {
        ...
        md.push_str("# Anvil Context Summary\n\n");
...
impl AgentAdapter for ClaudeCodeAdapter {
    ...
    fn adapt(&self, context: &AnvilContext, _exporter: &dyn ContextExporter) -> Result<String, String> {
        let mut xml = String::new();
        xml.push_str("<anvil_context>");
...
impl AgentAdapter for ContinueAdapter {
    ...
    fn adapt(&self, context: &AnvilContext, exporter: &dyn ContextExporter) -> Result<String, String> {
        ...
        let items = serde_json::json!([
            {
                "name": "Anvil Context",
                "description": "Active Anvil environment runtimes and workspace information",
                "content": md
            }
        ]);
...
mod tests {
    ...
    fn test_handshake_version_match() {
        ...
        let response: AcpHandshakeResponse = serde_json::from_str(&res_str).unwrap();
    ...
    fn test_secret_masking() {
        ...
        std::env::set_var("ANVIL_TEST_DATABASE_PASSWORD", "supersecret123");
        std::env::set_var("ANVIL_TEST_NORMAL_VAR", "hello-world");

        let options = ContextOptions::default();
        let result = env_provider.collect(&options).unwrap();
        assert_eq!(result.get("ANVIL_TEST_DATABASE_PASSWORD").unwrap().as_str().unwrap(), "[MASKED]");
        assert_eq!(result.get("ANVIL_TEST_NORMAL_VAR").unwrap().as_str().unwrap(), "hello-world");

        std::env::remove_var("ANVIL_TEST_DATABASE_PASSWORD");
        std::env::remove_var("ANVIL_TEST_NORMAL_VAR");
    }
    ...
    fn test_register_plugin_exporter_duplicate_rejection() {
        ...
        impl ContextExporter for FirstExporter {
            ...
            fn export(&self, _context: &AnvilContext) -> Result<String, String> {
                Ok("first".to_string())
            }
        }
    ...
    fn test_workspace_limit_bounds() {
        let temp_dir = std::env::temp_dir().join("anvil_workspace_test");
