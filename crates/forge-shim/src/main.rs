...
fn main() {
    let current_exe = match std::env::current_exe() {
        Ok(path) => path,
        Err(e) => {
            eprintln!("Forge Shim Error: Failed to resolve current executable path: {}", e);
            std::process::exit(1);
        }
    };
    ...
    let tool_name = match current_exe.file_stem().and_then(|s| s.to_str()) {
        ...
        None => {
            eprintln!("Forge Shim Error: Current executable has no valid name.");
            std::process::exit(1);
        }
    };
    ...
    let target_path = match target_binary {
        ...
        None => {
            ...
            match find_fallback_in_path(&tool_name, path_str) {
                Some(path) => path,
                None => {
                    eprintln!("Python/Node/etc is not available. Forge did not find a local config or a global install. Run 'forge init' or 'forge install <tool>'.");
                    std::process::exit(1);
                }
            }
        }
...
fn find_shims_cache() -> Option<PathBuf> {
    if let Ok(start_dir) = std::env::current_dir() {
        ...
        loop {
            let candidate = current.join(".forge").join("shims.cache");
            if candidate.exists() {
...
fn read_shims_cache(path: &Path, tool_name: &str) -> Option<PathBuf> {
    ...
    if !content.starts_with("# forge-shims-cache-v1") {
        return None;
    }
...
fn execute_process(target_path: &Path, filtered_path: Option<&str>) {
    ...
    #[cfg(unix)]
    {
        ...
        let err = cmd.exec();
        eprintln!("Forge Shim Error: Failed to execute process: {}", err);
        std::process::exit(1);
    }
    ...
    #[cfg(not(unix))]
    {
        let mut child = match cmd.spawn() {
            Ok(c) => c,
            Err(e) => {
                eprintln!("Forge Shim Error: Failed to spawn process: {}", e);
                std::process::exit(1);
            }
        };
        ...
        let status = match child.wait() {
            Ok(s) => s,
            Err(e) => {
                eprintln!("Forge Shim Error: Failed to wait for process: {}", e);
                std::process::exit(1);
            }
        };
...
mod tests {
    ...
    #[test]
    fn test_filter_path() {
        let temp_dir = std::env::temp_dir().join("forge_shim_test_filter_path");
    ...
    fn test_cache_invalidation_incorrect_header() {
        let temp_dir = std::env::temp_dir().join("forge_shim_test_cache_invalidation");
        fs::create_dir_all(&temp_dir).unwrap();
        ...
        fs::write(&cache_file, "# forge-shims-cache-v2\nnode = /path/to/node\n").unwrap();
        assert_eq!(read_shims_cache(&cache_file, "node"), None);
