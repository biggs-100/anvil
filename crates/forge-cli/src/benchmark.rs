...
pub async fn benchmark_sync(current_dir: &Path) -> BenchmarkResult {
    ...
    match forge_core::Engine::new(current_dir.to_path_buf()) {
...
pub async fn benchmark_diagnostics(current_dir: &Path) -> BenchmarkResult {
    let cache_dir = match forge_core::get_cache_dir() {
        Ok(d) => d,
        Err(e) => {
    ...
    let start = Instant::now();
    let engine = forge_core::DiagnosticEngine::new();
...
pub async fn benchmark_context(current_dir: &Path) -> BenchmarkResult {
    ...
    let active_profile = toml_path.as_ref().and_then(|path| {
        std::env::var("FORGE_PROFILE")
            .ok()
            .or_else(|| forge_core::load_config(path).ok().and_then(|c| c.profile.and_then(|p| p.keys().next().cloned())))
    });
...
mod tests {
    ...
    fn test_error_rendering_in_table() {
        let results = vec![BenchmarkResult {
            ...
            duration_ms: None,
            health_score: None,
            error: Some("Engine::new failed: no forge.toml".to_string()),
        }];
        let output = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            print_results_table(&results);
        }));
