//! Integration tests for anvil mcp.
//!
//! These tests spawn the anvil binary with the `mcp` subcommand,
//! send JSON-RPC 2.0 requests over stdin, and verify responses from stdout.
//!
//! Run with: cargo test --test mcp_test -- --nocapture
//!
//! Cargo automatically sets CARGO_BIN_EXE_ANVIL when the test binary is
//! built in the same workspace as the anvil crate. No --ignored flag needed.

use std::io::{BufRead, BufReader, Write};
use std::process::{Command, Stdio};

/// Locate the anvil binary using Cargo's env var or fallback.
///
/// Returns the path to the anvil binary.
///
/// Integration tests in the same crate don't get CARGO_BIN_EXE_*,
/// so we derive the path from the test binary location.
fn anvil_exe() -> std::path::PathBuf {
    let current_exe = std::env::current_exe().expect("test binary path");
    // The test binary is in target/debug/deps/anvil_cli-<hash>.exe
    // The anvil binary is in target/debug/anvil.exe
    let target_dir = current_exe.parent() // deps/
        .and_then(|p| p.parent())         // debug/
        .expect("target directory");
    let exe_name = if cfg!(windows) { "anvil.exe" } else { "anvil" };
    target_dir.join(exe_name)
}

/// Helper: send a sequence of MCP requests and collect all response lines.
///
/// Spawns `anvil mcp`, writes each request line to stdin, then closes stdin
...
fn send_mcp_requests(requests: &[&str]) -> Vec<String> {
    ...
    let mut child = Command::new(&exe)
        ...
        .spawn()
        .expect("failed to spawn anvil mcp");
...
#[test]
fn test_list_resources_returns_anvil_context() {
    let lines = send_mcp_requests(&[
        r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocol_version":"2024-11-05","capabilities":{},"client_info":{"name":"test","version":"1.0"}}}"#,
    ...
    assert_eq!(resources[0]["uri"], "anvil://context/active");
}
...
fn test_list_prompts_returns_three() {
    ...
    assert!(names.contains(&"anvil:status"));
    assert!(names.contains(&"anvil:diagnose"));
    assert!(names.contains(&"anvil:explain"));
}
...
fn test_read_resource_active_context() {
    let lines = send_mcp_requests(&[
        r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocol_version":"2024-11-05","capabilities":{},"client_info":{"name":"test","version":"1.0"}}}"#,
        r#"{"jsonrpc":"2.0","id":2,"method":"resources/read","params":{"uri":"anvil://context/active"}}"#,
    ]);
    ...
    assert_eq!(contents[0]["uri"], "anvil://context/active");
