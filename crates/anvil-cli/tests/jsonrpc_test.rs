//! Integration tests for anvil jsonrpc.
//!
//! These tests spawn the anvil binary with the `jsonrpc` subcommand,
...
///
/// Cargo sets `CARGO_BIN_EXE_ANVIL` (uppercase) when integration tests run
/// in a workspace that produces the `anvil` binary. The fallback is used when
/// running outside of Cargo (e.g., IDE test runner).
...
/// Helper: spawn `anvil jsonrpc`, send a request, read one response line.
fn send_request(request: &str) -> String {
    ...
    let mut child = Command::new(&exe)
        ...
        .spawn()
        .expect("failed to spawn anvil jsonrpc");
...
fn test_subprocess_lifecycle_error() {
    /// When the anvil subprocess dies mid-request, the client should
    ...
    child.kill().expect("failed to kill anvil process");
