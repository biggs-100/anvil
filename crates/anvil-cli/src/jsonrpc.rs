...
//!
//! Runs as the `anvil jsonrpc` subcommand. Reads one JSON-RPC request per line
...
use anvil_core::operations::{RunOperation, ShellOperation};
use anvil_core::types::OperationResult;
use anvil_core::{ContextExporter, Engine, Operation};
...
async fn handle_context_get(current_dir: &Path, params: &Value) -> Result<Value, String> {
    ...
    let mut ctx_engine = anvil_core::ContextEngine::new();
