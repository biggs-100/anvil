...
pub fn read_audit_log(workspace_root: &Path) -> Result<Vec<AuditEntry>, String> {
    let journal_path = workspace_root.join(".anvil").join("journal.jsonl");
...
mod tests {
    ...
    fn test_filter_relevant_phases() {
        ...
        let temp_dir = std::env::temp_dir().join("anvil_test_audit_filter");
        let _ = std::fs::remove_dir_all(&temp_dir);
        std::fs::create_dir_all(&temp_dir.join(".anvil")).unwrap();

        let journal_path = temp_dir.join(".anvil").join("journal.jsonl");
        {
    ...
    fn test_empty_journal_returns_empty() {
        let temp_dir = std::env::temp_dir().join("anvil_test_audit_empty");
        let _ = std::fs::remove_dir_all(&temp_dir);
