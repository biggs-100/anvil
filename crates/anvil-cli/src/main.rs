...
use serde_json::json;
use anvil_core::operations::{
    InitOperation, ResolveOperation, LockOperation, GcOperation,
    RunOperation, ShellOperation, PlanOperation, ValidateOperation,
};
use anvil_core::api::v1::OperationSummary;
use anvil_core::context::{ContextEngine, ContextOptions, AnvilContext};
use anvil_core::manifest::AnvilConfig;
use anvil_core::secrets::{ResolvedEnvironment, ValueSource};
use anvil_core::{CliCommand, ContextExporter, AgentAdapter, PluginRegistry, PolicyEngine};
...
#[derive(Parser)]
#[command(name = "anvil", version = "0.1.0", about = "Reproducible development environments — for humans, tools, and AI agents")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    #[command(about = "Initialize anvil in the current directory")]
    Init,
    #[command(about = "Resolve runtime versions")]
    Resolve,
    #[command(about = "Generate or update anvil.lock")]
    Lock,
    ...
    #[command(about = "Create a portable .anvil archive of anvil descriptors")]
    Bundle {
        #[arg(long, help = "Custom output path (default: <project_dir_name>.anvil)")]
        output: Option<PathBuf>,
    },
    #[command(about = "Restore anvil descriptors from a .anvil archive")]
    Restore {
        path: PathBuf,
    ...
    #[command(about = "Manage remote registry and ARRS metadata cache")]
    Registry {
        #[command(subcommand)]
...
enum RegistryCommands {
    #[command(about = "Clear ARRS metadata cache and re-fetch from remote registry")]
    Refresh,
}
...
async fn run_operation<O: anvil_core::operations::Operation + 'static>(
    op: O,
    current_dir: &Path,
    event_bus: &anvil_core::event_bus::EventBus,
) -> Result<(), String> {
    let cache_dir = anvil_core::get_cache_dir()?;
    let workspace_root = anvil_core::find_anvil_toml(current_dir)
        .and_then(|p| p.parent().map(|p| p.to_path_buf()))
        .unwrap_or_else(|| current_dir.to_path_buf());

    let mut ctx = anvil_core::operations::Context::new(workspace_root.clone(), cache_dir, event_bus.clone());
    ...
    let final_state = anvil_core::compute_current_state(&workspace_root, &ctx.cache_dir);
    anvil_core::save_state(&workspace_root, final_state);

    if result.status == anvil_core::types::OperationStatus::Failure {
...
fn print_history_table(history: &[anvil_core::api::v1::OperationSummary]) {
...
fn print_explain_table(exp: &anvil_core::api::v1::RuntimeExplanation) {
...
/// Type-safe enum for anvil explain subcommands.
/// Parsed manually from Vec<String> to maintain backward compat
/// with `anvil explain <runtime>` (no subcommand keyword needed).
#[derive(Debug)]
        ...
        match args.first() {
            Some(s) if s == "operation" || s == "op" => {
                let id = args.get(1).ok_or_else(|| {
                    "Usage: anvil explain operation <id>".to_string()
                })?;
            ...
            None => Err(
                "Missing runtime name or subcommand. Usage: anvil explain <runtime> [subcommand]"
                    .to_string(),
            ),
        }
    }
...
/// Top-level dispatch for `anvil explain ...`.
...
async fn handle_explain(current_dir: &Path, args: &[String]) -> Result<(), String> {
    ...
    match subcommand {
        ExplainSubcommand::Runtime { name } => {
            let engine = anvil_core::Engine::new(current_dir.to_path_buf())?;
...
async fn explain_operation(engine: &anvil_core::Engine, id: &str) -> Result<(), String> {
...
async fn explain_context(current_dir: &Path) -> Result<(), String> {
    ...
    let active_profile = if let Some(ref path) = toml_path {
        std::env::var("ANVIL_PROFILE").ok().or_else(|| {
            anvil_core::load_config(path).ok().and_then(|c| {
...
fn print_context_table(ctx: &AnvilContext) {
...
async fn explain_config(engine: &anvil_core::Engine, current_dir: &Path) -> Result<(), String> {
    let resolved = engine.env_resolve(None).await?;

    let toml_path = anvil_core::find_anvil_toml(current_dir);
    let has_secret_defs = toml_path.as_ref().and_then(|p| {
        anvil_core::load_config(p).ok().and_then(|c| c.config)
    });
...
fn print_config_table(resolved: &ResolvedEnvironment, config_section: Option<&anvil_core::manifest::ConfigSection>) {
...
async fn explain_profile(current_dir: &Path) -> Result<(), String> {
    let toml_path = anvil_core::find_anvil_toml(current_dir)
        .ok_or_else(|| "No anvil.toml found".to_string())?;
    let config = anvil_core::load_config(&toml_path)?;

    let active_name = get_active_profile(current_dir)
        .ok_or_else(|| "No active profile found. Set ANVIL_PROFILE or add a [profile] section to anvil.toml".to_string())?;
...
fn print_profile_table(active_name: &str, config: &AnvilConfig, resolved: &ResolvedEnvironment) {
...
async fn run_cli(cli: Cli) -> Result<(), String> {
    ...
    let event_bus = anvil_core::event_bus::EventBus::new(100);
    ...
    tokio::spawn(async move {
        while let Ok(event) = rx.recv().await {
            ...
            match event.status {
                anvil_core::types::EventStatus::Started => {
                    println!("[+] [{}] Starting phase: {}", event.runtime, event.phase);
                }
                anvil_core::types::EventStatus::Progress(pct) => {
                    println!("[~] [{}] Progress: {}% - {}", event.runtime, pct, msg);
                }
                anvil_core::types::EventStatus::Finished => {
                    println!("[*] [{}] Finished: {}", event.runtime, msg);
                }
                anvil_core::types::EventStatus::Failed(err) => {
    ...
    match cli.command {
        ...
        Commands::Sync => {
            ...
            let engine = anvil_core::Engine::new(current_dir.clone())?;
            engine.sync().await?;
        ...
        Commands::Tui => {
            ...
            let diag_ctx = anvil_core::DiagnosticContext {
                ...
                cache_dir,
                mode: anvil_core::DiagnosticMode::Deep,
                active_profile: None,
            };
            anvil_tui::App::run(engine, diag_ctx, plugin_health_checks).await?;
        }
        ...
        Commands::Context { format, scope, exclude } => {
            ...
            match format.as_str() {
                "json" => {
                    let exporter = anvil_core::JsonExporter { pretty: false };
                    let output = exporter.export(&context)?;
                    println!("{}", output);
                }
                "json-pretty" | "pretty" => {
                    let exporter = anvil_core::JsonExporter { pretty: true };
                    let output = exporter.export(&context)?;
                    println!("{}", output);
                }
                "markdown" | "md" => {
                    let exporter = anvil_core::MarkdownExporter;
                    let output = exporter.export(&context)?;
                    println!("{}", output);
                }
                "mcp" => {
                    let exporter = anvil_core::McpExporter;
                    let output = exporter.export(&context)?;
                    println!("{}", output);
                }
                "claude" => {
                    let exporter = anvil_core::JsonExporter { pretty: false };
                    let adapter = anvil_core::ClaudeCodeAdapter;
                    let output = adapter.adapt(&context, &exporter)?;
                    println!("{}", output);
                }
                "gemini" => {
                    let exporter = anvil_core::JsonExporter { pretty: false };
                    let adapter = anvil_core::GeminiCliAdapter;
                    let output = adapter.adapt(&context, &exporter)?;
                    println!("{}", output);
                }
                "aider" => {
                    let exporter = anvil_core::JsonExporter { pretty: false };
                    let adapter = anvil_core::AiderAdapter;
                    let output = adapter.adapt(&context, &exporter)?;
                    println!("{}", output);
                }
                "continue" => {
                    let exporter = anvil_core::MarkdownExporter;
                    let adapter = anvil_core::ContinueAdapter;
                    let output = adapter.adapt(&context, &exporter)?;
                    println!("{}", output);
                }
                other => {
        ...
        Commands::Registry { subcommand } => {
            match subcommand {
                RegistryCommands::Refresh => {
                    ...
                    let frrs_cache_dir = workspace_root.join(".anvil").join("metadata_cache");
                    let registry_url = std::env::var("ANVIL_REGISTRY_URL")
                        .unwrap_or_else(|_| "https://registry.anvil.dev".to_string());

                    if registry_url.is_empty() {
                        eprintln!("Error: Remote registry is disabled (ANVIL_REGISTRY_URL is empty).");
                        std::process::exit(1);
                    }

                    // Clear existing cache directory
                    if frrs_cache_dir.exists() {
                        println!("Clearing ARRS metadata cache...");
                        std::fs::remove_dir_all(&frrs_cache_dir)
                            .map_err(|e| format!("Failed to clear cache: {}", e))?;
                    }

                    let remote = anvil_core::RemoteRegistry::new(&registry_url, frrs_cache_dir.clone());
        ...
        Commands::Bundle { output } => {
            ...
            let output_path = match output {
                ...
                None => {
                    ...
                    workspace_root.join(format!("{}.anvil", dir_name))
                }
            };

            anvil_core::bundle::create_bundle(&workspace_root, &output_path)
        ...
        Commands::Restore { path, force } => {
            ...
            if !force {
                let toml_path = workspace_root.join("anvil.toml");
                if toml_path.exists() {
                    return Err("anvil.toml already exists. Use --force to overwrite.".to_string());
                }
            }

            anvil_core::bundle::restore_bundle(&path, &workspace_root)
                .map_err(|e| format!("Failed to restore bundle: {}", e))?;

            println!("Bundle restored successfully from: {}", path.display());

            // Delegate to `anvil up` to download/sync runtimes
            println!("Syncing runtimes...");
            let engine = anvil_core::Engine::new(current_dir.clone())?;
            engine.sync().await?;
        }
        Commands::Snapshot { subcommand } => {
            let workspace_root = anvil_core::find_anvil_toml(&current_dir)
                ...
                .unwrap_or_else(|| current_dir.clone());
            let cache_dir = anvil_core::get_cache_dir()?;
            let manager = anvil_core::SnapshotManager::new(&workspace_root, &cache_dir);
            ...
            match subcommand {
                SnapshotCommands::Create { name, description } => {
                    ...
                    let toml_path = workspace_root.join("anvil.toml");
                    if !toml_path.exists() {
                        return Err("anvil.toml not found. Run 'anvil init' first.".to_string());
                    }
                    ...
                    if !dry_run {
                        ...
                        match run_operation(LockOperation, &current_dir, &event_bus).await {
                            ...
                            Err(e) => {
                                ...
                                let bak_toml = workspace_root.join("anvil.toml.bak");
                                let bak_lock = workspace_root.join("anvil.lock.bak");
                                if bak_toml.exists() {
                                    let _ = std::fs::copy(
                                        &bak_toml,
        ...
        Commands::PluginCommand(args) => {
            ...
            if let Some(cmd) = plugin_cli_commands.iter().find(|c| c.name() == cmd_name) {
                ...
                if known.is_empty() {
                    return Err(format!("Unknown command: '{}'. Run 'anvil --help' for available commands.", cmd_name));
                } else {
                    return Err(format!(
            ...
            match subcommand {
                AiCommands::Context => {
                    ...
                    if let Some(ref path) = toml_path {
                        if let Ok(config) = anvil_core::load_config(path) {
                            active_runtimes = config.runtimes;
                        }
                    }
                    
                    let env_vars = if let Some(env_path) = anvil_core::find_anvil_env(&current_dir) {
                        let parsed = anvil_core::parse_env_file(&env_path)?;
                        anvil_core::mask_env_vars(&parsed)
                    } else {
                        HashMap::new()
                    };
...
/// If `anvil.toml` has a `[packages]` section with pip configured, run
/// `pip install -r <requirements>` using the anvil-managed python binary.
///
/// Error handling:
...
fn install_packages_if_configured(current_dir: &Path) -> Result<(), String> {
    let toml_path = match anvil_core::find_anvil_toml(current_dir) {
        Some(p) => p,
        None => return Ok(()),
    };

    let config = match anvil_core::load_config(&toml_path) {
        Ok(c) => c,
        Err(_) => return Ok(()),
    };
    ...
    match anvil_core::install_pip_deps(packages_config, &workspace_root, &cache_dir) {
        Ok(()) => Ok(()),
        Err(e) if e.contains("No python runtime found") => {
            eprintln!("Warning: pip install skipped — no python runtime available. Run 'anvil up' to sync runtimes first.");
            Ok(())
        }
...
/// Build a [`PolicyEngine`] from `anvil.toml` if a `[policy]` section exists.
/// Returns `None` when no `anvil.toml` is found or the section is absent.
fn build_policy_engine(current_dir: &Path) -> Option<PolicyEngine> {
    let toml_path = anvil_core::find_anvil_toml(current_dir)?;
    let config = anvil_core::load_config(&toml_path).ok()?;
...
fn compute_lockfile_state(current_dir: &Path) -> (bool, bool) {
    let workspace_root = anvil_core::find_anvil_toml(current_dir)
        .and_then(|p| p.parent().map(|p| p.to_path_buf()))
        .unwrap_or_else(|| current_dir.to_path_buf());
    let lockfile_path = workspace_root.join("anvil.lock");
    if !lockfile_path.exists() {
        return (false, false);
    }
    match anvil_core::load_lockfile(&lockfile_path) {
        Ok(lockfile) => {
...
fn compute_health_score(
    current_dir: &Path,
    plugin_health_checks: &[std::sync::Arc<dyn anvil_core::HealthCheck>],
) -> u8 {
    match anvil_core::get_cache_dir() {
        Ok(cache_dir) => {
            let diag_ctx = anvil_core::DiagnosticContext {
                workspace_root: current_dir.to_path_buf(),
                cache_dir,
...
/// Get the active profile name (from `ANVIL_PROFILE` env var or first profile key).
...
/// Get the list of active runtime names from `anvil.toml`.
fn get_active_runtimes(current_dir: &Path) -> Vec<String> {
    let toml_path = match anvil_core::find_anvil_toml(current_dir) {
        Some(p) => p,
        None => return Vec::new(),
    };
    match anvil_core::load_config(&toml_path) {
...
fn enforce_policy(violations: Vec<anvil_core::PolicyViolation>) {
...
fn get_shim_binary_path() -> Result<PathBuf, String> {
    ...
    let name = if cfg!(windows) { "anvil-shim.exe" } else { "anvil-shim" };
    ...
    Err(format!(
        "Could not find anvil-shim executable. Looked in {:?} and {:?}",
        candidate, target_debug
    ))
}

fn get_home_dir() -> Option<PathBuf> {
    if let Some(anvil_home) = std::env::var_os("ANVIL_HOME") {
        Some(PathBuf::from(anvil_home))
    } else {
        dirs::home_dir()
    }
}
...
fn setup_shims() -> Result<(), String> {
    ...
    let dest_dir = home.join(".anvil").join("bin");
    ...
    println!("Anvil shims setup complete. Placed in {:?}", dest_dir);
    Ok(())
}
...
fn uninstall_shims() -> Result<(), String> {
    ...
    println!("Anvil shims uninstalled successfully.");
    Ok(())
}

fn run_diagnostic_engine(
    ctx: &anvil_core::DiagnosticContext,
    plugin_health_checks: &[std::sync::Arc<dyn anvil_core::HealthCheck>],
) -> anvil_core::DiagnosticReport {
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => {
            tokio::task::block_in_place(|| {
                handle.block_on(async {
                    let mut engine = anvil_core::DiagnosticEngine::new();
...
fn render_finding_table(report: &anvil_core::DiagnosticReport) {
    ...
    for f in &report.findings {
        let sev = match f.severity {
            anvil_core::Severity::INFO => "INFO",
            anvil_core::Severity::WARNING => "WARNING",
            anvil_core::Severity::ERROR => "ERROR",
            anvil_core::Severity::CRITICAL => "CRITICAL",
        };
...
fn run_doctor(current_dir: &Path, deep: bool, json: bool, plugin_health_checks: &[std::sync::Arc<dyn anvil_core::HealthCheck>]) -> Result<(), String> {
    let cache_dir = anvil_core::get_cache_dir()?;
    let mode = if deep {
        anvil_core::DiagnosticMode::Deep
    } else {
        anvil_core::DiagnosticMode::Fast
    };
    let diag_ctx = anvil_core::DiagnosticContext {
        workspace_root: current_dir.to_path_buf(),
        cache_dir,
...
fn run_which(current_dir: &Path, runtime: &str) -> Result<(), String> {
    ...
    if let Some(ref path) = toml_path {
        let cache_file = path.parent().unwrap().join(".anvil").join("shims.cache");
        ...
        if source == "local" {
            let lock_file_path = path.parent().unwrap().join("anvil.lock");
            if let Ok(lockfile) = anvil_core::load_lockfile(&lock_file_path) {
    ...
    // 2. If not local, check global PATH (filtering out ~/.anvil/bin)
    if resolved_path.is_none() {
        let home = get_home_dir().unwrap_or_default();
        let shim_dir = home.join(".anvil").join("bin");
...
mod tests {
    ...
    fn test_shim_args_and_exit_code_propagation() {
        let temp_dir = std::env::temp_dir().join("anvil_cli_integration_test");
        ...
        fs::write(
            &cache_file,
            format!("# anvil-shims-cache-v1\nmytool = {}\n", mock_target.to_str().unwrap().replace('\\', "/")),
        )
        .unwrap();

        fs::write(temp_dir.join("anvil.toml"), "[runtimes]\n").unwrap();

        let current_exe = std::env::current_exe().unwrap();
        let build_dir = current_exe.parent().unwrap();
        let shim_exe_name = if cfg!(windows) { "anvil-shim.exe" } else { "anvil-shim" };
        ...
        if !shim_path.exists() {
            panic!("Could not find anvil-shim compiled executable at {:?}", shim_src);
        }
    ...
    fn test_setup_and_uninstall_shims() {
        let temp_dir = std::env::temp_dir().join("anvil_cli_setup_uninstall_test");
        fs::create_dir_all(&temp_dir).unwrap();

        // Override ANVIL_HOME env var to use temp_dir
        let old_anvil_home = std::env::var("ANVIL_HOME").ok();
        std::env::set_var("ANVIL_HOME", &temp_dir);

        // Run setup
        let res_setup = setup_shims();
        assert!(res_setup.is_ok(), "setup_shims failed: {:?}", res_setup);

        let bin_dir = temp_dir.join(".anvil").join("bin");
        ...
        // Restore env vars
        if let Some(val) = old_anvil_home {
            std::env::set_var("ANVIL_HOME", val);
        } else {
            std::env::remove_var("ANVIL_HOME");
        }
    ...
    fn test_plugin_cli_command_dispatch() {
        use anvil_core::{CliCommand, ExtensionSink, Plugin, PluginRegistry};
    ...
    /// 5.6: Backward compat — `anvil explain node` parses as Runtime { name: "node" }
    #[test]
    ...
    fn test_explain_parse_operation_missing_id() {
        ...
        assert!(err.contains("Usage: anvil explain operation"));
    }
    ...
    fn test_print_config_table() {
        use anvil_core::manifest::ConfigSection;
        use anvil_core::secrets::VarMetadata;
        ...
        definitions.insert(
            "API_KEY".to_string(),
            anvil_core::manifest::ConfigDefinition {
    ...
    fn test_print_context_table() {
        ...
        let ctx = AnvilContext {
            ...
            config: json!({"anvil.toml": "/test/anvil.toml"}),
