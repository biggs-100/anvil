...
#![allow(dead_code)]
//!
//! Runs as the `anvil mcp` subcommand. Implements the Model Context Protocol,
...
//!   │                               │
//!   │◄── anvil/state_changed ───────│── background EventBus subscriber
//!   │◄── anvil/error ───────────────│
//! ```
//!
...
use anvil_core::operations::{RunOperation, ShellOperation, PlanOperation};
use anvil_core::types::OperationResult;
use anvil_core::{ContextExporter, Operation};
use anvil_core::{Engine, Event, EventBus, ContextEngine, ContextOptions, McpExporter};
use anvil_core::{DiagnosticEngine, DiagnosticContext, DiagnosticMode};
...
async fn notification_loop(
    ...
    // Track last known engine state for anvil/state_changed
    let mut last_state: Option<String> = None;

    loop {
        match rx.recv().await {
            Ok(event) => {
                // ── anvil/state_changed: on lifecycle phase transitions ──
                let current = Some(event.runtime.clone());
                if last_state != current {
                    if let Some(ref old) = last_state {
                        let notif = McpNotification {
                            jsonrpc: "2.0".to_string(),
                            method: "anvil/state_changed".to_string(),
                            params: json!({
                                "old_state": old,
                ...
                // ── anvil/error: on operation failure ──
                if let anvil_core::types::EventStatus::Failed(ref err) = event.status {
                    let notif = McpNotification {
                        jsonrpc: "2.0".to_string(),
                        method: "anvil/error".to_string(),
                        params: json!({
                ...
                // ── anvil/warning: on warning in diagnostics or low health ──
                if let Some(ref msg) = event.message {
                        ...
                        || msg.to_lowercase().contains("critical")
                    {
                        let notif = McpNotification {
                            jsonrpc: "2.0".to_string(),
                            method: "anvil/warning".to_string(),
                            params: json!({
                                "finding": msg,
                                "severity": "WARNING",
                            }),
...
async fn handle_list_tools() -> Result<Value, String> {
    let tools = vec![
        ToolDescription {
            name: "anvil_run".to_string(),
            description: "Execute a command inside the activated anvil environment".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolDescription {
            name: "anvil_shell".to_string(),
            description: "Spawn an interactive subshell inside the anvil environment".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {}
            }),
        ...
        ToolDescription {
            name: "anvil_doctor".to_string(),
            description: "Run diagnostics checks on the anvil environment".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "mode": {
...
async fn handle_call_tool(
    ...
    let (content, is_error) = match request.name.as_str() {
        "anvil_run" => {
            ...
            let mut ctx = anvil_core::operations::Context::new(
                current_dir.to_path_buf(),
                anvil_core::get_cache_dir()?,
                event_bus.clone(),
            );
            ...
            let is_err = result.status == anvil_core::types::OperationStatus::Failure;
...
async fn handle_list_resources() -> Result<Value, String> {
    let result = ListResourcesResult {
        resources: vec![ResourceDescription {
            uri: "anvil://context/active".to_string(),
            name: "Active Anvil Context".to_string(),
            mime_type: "application/json".to_string(),
            description: Some("Complete serialized Anvil environment context including runtimes, configuration, diagnostics, workspace, environment, and secrets metadata".to_string()),
        }],
    };
    serde_json::to_value(&result)
...
async fn handle_read_resource(current_dir: &Path, params: &Value) -> Result<Value, String> {
    ...
    if request.uri != "anvil://context/active" {
    ...
    ctx_engine.register(std::sync::Arc::new(anvil_core::RuntimeProviderImpl));
    ctx_engine.register(std::sync::Arc::new(anvil_core::ConfigurationProviderImpl));
    ctx_engine.register(std::sync::Arc::new(anvil_core::DiagnosticsProviderImpl));
    ctx_engine.register(std::sync::Arc::new(anvil_core::WorkspaceProviderImpl));
    ctx_engine.register(std::sync::Arc::new(anvil_core::EnvironmentProviderImpl));
    ctx_engine.register(std::sync::Arc::new(anvil_core::SecretsProviderImpl));
    ...
    // { "contents": [{ "uri": "anvil://context/active", "mimeType": "application/json", "text": "..." }] }
    ...
    let result = ReadResourceResult {
        contents: contents
            ...
            .map(|item| ResourceContents {
                uri: item.get("uri").and_then(|u| u.as_str()).unwrap_or("anvil://context/active").to_string(),
...
async fn handle_list_prompts() -> Result<Value, String> {
    let result = ListPromptsResult {
        prompts: vec![
            PromptDescription {
                name: "anvil:status".to_string(),
                description: "Current anvil environment state summary including lifecycle status, runtimes, and health".to_string(),
                arguments: None,
            },
            PromptDescription {
                name: "anvil:diagnose".to_string(),
                description: "Run diagnostics and return a health report with findings, explanations, and suggested fixes".to_string(),
                arguments: None,
            },
            PromptDescription {
                name: "anvil:explain".to_string(),
...
async fn handle_get_prompt(engine: &Engine, params: &Value) -> Result<Value, String> {
    ...
    let markdown = match request.name.as_str() {
        "anvil:status" => {
            ...
            format!(
                "# Anvil Environment Status\n\n\
                 ## Lifecycle State\n\
                 {}\n\n\
                 ## Overview\n\
                 The anvil environment manages development runtimes and configuration. \
                 Run `anvil doctor` for detailed health checks.\n",
                state,
            )
        }
        "anvil:diagnose" => {
            ...
            md.push_str("# Anvil Diagnostics Report\n\n");
...
mod tests {
    ...
    fn test_initialize_result_serialization() {
        ...
        assert!(json_str.contains("\"anvil\""));
    }
    ...
    fn test_list_tools_serialization() {
        ...
        let json_str = serde_json::to_string(&result).unwrap();
        assert!(json_str.contains("\"name\":\"anvil_run\""));
    ...
    fn test_list_tools_six_tools() {
        ...
        assert!(names.contains(&"anvil_shell"));
        assert!(names.contains(&"anvil_sync"));
        assert!(names.contains(&"anvil_plan"));
        assert!(names.contains(&"anvil_explain"));
        assert!(names.contains(&"anvil_doctor"));
    }

    #[test]
    fn test_call_tool_request_deserialize() {
        let raw = r#"{"name":"anvil_run","arguments":{"cmd":"echo","args":["hello"]}}"#;
        let req: CallToolRequest = serde_json::from_str(raw).unwrap();
        assert_eq!(req.name, "anvil_run");
    ...
    fn test_read_resource_request_deserialize() {
        let raw = r#"{"uri":"anvil://context/active"}"#;
        let req: ReadResourceRequest = serde_json::from_str(raw).unwrap();
        assert_eq!(req.uri, "anvil://context/active");
    }
    ...
    fn test_read_resource_result_serialization() {
        ...
        let json_str = serde_json::to_string(&result).unwrap();
        assert!(json_str.contains("\"uri\":\"anvil://context/active\""));
    ...
    fn test_list_resources_serialization() {
        ...
        let json_str = serde_json::to_string(&result).unwrap();
        assert!(json_str.contains("\"anvil://context/active\""));
    }
    ...
    fn test_list_prompts_serialization() {
        ...
        let json_str = serde_json::to_string(&result).unwrap();
        assert!(json_str.contains("anvil:status"));
        assert!(json_str.contains("anvil:diagnose"));
        assert!(json_str.contains("anvil:explain"));
    }

    #[test]
    fn test_get_prompt_request_deserialize() {
        let raw = r#"{"name":"anvil:status"}"#;
        let req: GetPromptRequest = serde_json::from_str(raw).unwrap();
        assert_eq!(req.name, "anvil:status");
    }
    ...
    fn test_mcp_notification_serialization() {
        ...
        assert!(json_str.contains("\"method\":\"anvil/state_changed\""));
